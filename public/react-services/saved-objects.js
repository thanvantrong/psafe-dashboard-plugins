/*
 * Psafe app - Saved Objects management service
...
export class SavedObject {
   ...
   */
  static async getListOfPsafeValidIndexPatterns(defaultIndexPatterns, where) {
    let result = [];
  ...
  static async existsOrCreateIndexPattern(patternID) {
    ...
    if (!result.data) {
      let fields = '';
      if (satisfyPluginPlatformVersion('<7.11')) {
        fields = await SavedObject.getIndicesFields(patternID, PSAFE_INDEX_TYPE_ALERTS);
      }
  ...
  static async refreshIndexPattern(pattern, newFields = null) {
    try {
      const fields = await SavedObject.getIndicesFields(pattern.title, PSAFE_INDEX_TYPE_ALERTS);
  ...
  /**
   * Creates the 'psafe-alerts-*'  index pattern
   */
  static async createPsafeIndexPattern(pattern) {
    try {
      const fields = satisfyPluginPlatformVersion('<7.11')
        ? await SavedObject.getIndicesFields(pattern, PSAFE_INDEX_TYPE_ALERTS)
        : '';
      await this.createSavedObject(
        'index-pattern',
        pattern,
  ...
  static getIndicesFields = async (pattern, indexType) => {
    try {
      ...
      switch (indexType) {
        case PSAFE_INDEX_TYPE_MONITORING:
          return FieldsMonitoring;
        case PSAFE_INDEX_TYPE_STATISTICS:
          return FieldsStatistics;
        case PSAFE_INDEX_TYPE_ALERTS:
          return KnownFields;
      }
    }
  };
   ...
   * It is usefull to validate if the endpoint works as expected. Related issue: https://github.com/psafe/psafe-kibana-app/issues/4293
...
Restart the ${PLUGIN_PLATFORM_NAME} service to initialize the index. More information in <a href="${webDocumentationLink('user-manual/psafe-dashboard/troubleshooting.html#saved-object-for-index-pattern-not-found')}" target="_blank">troubleshooting</a>.`
)};
  }
}