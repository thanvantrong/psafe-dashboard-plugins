/*
 * Psafe app - Acntion Agents Service
...
export class ActionAgents {
  ...
  static async upgradeAgent(agentId) {
    try {
      await PaRequest.apiReq('PUT', `/agents/${agentId}/upgrade`, {
        force: 1,
      });
  ...
  static async upgradeAgents(selectedItems) {
    for (let item of selectedItems.filter(
      ...
      try {
        await PaRequest.apiReq('PUT', `/agents/${item.id}/upgrade`, '1');
  ...
  static async upgradeAllAgents(selectedItems, managerVersion) {
    selectedItems.forEach(async (agent) => {
      if (
        ...
        try {
          await PaRequest.apiReq('PUT', `/agents/${agent.id}/upgrade`, '1');
  ...
  static async restartAgent(agentId) {
    try {
      await PaRequest.apiReq('PUT', `/agents/restart`, { ids: agentId });
  ...
  static async restartAgents(selectedItems) {
    ...
    try {
      await PaRequest.apiReq('PUT', `/agents/restart`, { ids: [...agentsId] });
  ...
  static async restartAllAgents(selectedItems) {
    ...
    try {
      await PaRequest.apiReq('PUT', `/agents/restart`, { ids: [...idAvaibleAgents] });
  ...
  static async deleteAgents(selectedItems) {
    ...
    try {
      await PaRequest.apiReq('DELETE', `/agents`, {
        purge: true,
        ids: auxAgents,
        older_than: '1s',
      });
