/*
 * Psafe app - React hook for get query of plugin platform searchBar
...
import psafePermissions from '../../common/api-info/security-actions';

export class PaUserPermissions{
  ...
  static checkMissingUserPermissions = (requiredPermissions, userPermissions) => {
    const filtered = requiredPermissions.filter(permission => {
      if(Array.isArray(permission)){
        const missingOrPermissions = PaUserPermissions.checkMissingUserPermissions(permission, userPermissions);
      ...
      let actionResource = (typeof permission === 'string' && psafePermissions[actionName].resources.length === 1) ? (psafePermissions[actionName].resources[0] + ':*') : permission.resource;
      ...
      const existInPsafePermissions = (userResource) => {
        return !!psafePermissions[actionName].resources.find(function (resource) {
          return (
            resource ===
            userResource
              .split('&')
      ...
      const notAllowInPsafePermissions = (userResource) => {
        if (userResource !== RESOURCE_ANY) {
          return existInPsafePermissions(userResource)
      ...
      return userPermissions[actionName][actionResource]
        ? notAllowInPsafePermissions(actionResource)
