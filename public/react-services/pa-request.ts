...
import { ApiCheck } from './pa-api-check';
import { PaAuthentication } from './pa-authentication';
...
export class PaRequest {
  ...
  static async genericReq(
    ...
    try {
        ...
        if (currentApi && currentApi.id) {
          try {
            ...
            if (!window.location.hash.includes('#/settings')) {
              window.location.href = getHttp().basePath.prepend('/app/psafe#/health-check');
            }
      ...
      if (
        ...
        try {
          await PaAuthentication.refresh(true);
  ...
  /**
   * Perform a request to the Psafe API
