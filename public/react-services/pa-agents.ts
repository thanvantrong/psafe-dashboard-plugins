/*
 * Psafe app - Services related to agents
...
import { PSAFE_AGENTS_OS_TYPE } from '../../common/constants';
...
export function getAgentOSType(agent) {
  if (agent?.os?.uname?.toLowerCase().includes(PSAFE_AGENTS_OS_TYPE.LINUX)) {
    return PSAFE_AGENTS_OS_TYPE.LINUX;
  } else if (agent?.os?.platform === PSAFE_AGENTS_OS_TYPE.WINDOWS) {
    return PSAFE_AGENTS_OS_TYPE.WINDOWS;
  } else if (agent?.os?.platform === PSAFE_AGENTS_OS_TYPE.SUNOS) {
    return PSAFE_AGENTS_OS_TYPE.SUNOS;
  } else if (agent?.os?.platform === PSAFE_AGENTS_OS_TYPE.DARWIN) {
    return PSAFE_AGENTS_OS_TYPE.DARWIN;
  } else {
    return PSAFE_AGENTS_OS_TYPE.OTHERS;
  }
}
...
export async function getAuthorizedAgents() {
  try {
    ...
    const output: IApiResponse<{ id: string }> = await PaRequest.apiReq('GET', `/agents`, {});
    ...
    if (totalItems && output.data && output.data.data && totalItems > 500) {
      ...
      while (itemsArray.length < totalItems && params.offset < totalItems) {
        ...
        const tmpData: IApiResponse<{ id: string }> = await PaRequest.apiReq('GET', `/agents`, {
          params: params,
        });
