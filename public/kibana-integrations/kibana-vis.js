/*
 * Psafe app - React component for custom kibana visualizations.
...
class KibanaVis extends Component {
  ...
  componentWillUnmount() {
    if (this._isMounted) {
      ...
      // 
      // Related issue:
      // https://github.com/psafe/psafe-kibana-app/issues/4158
      if (this.deadField) {
  ...
  myRender = async (raw) => {
    ...
    try {
      ...
      const isAgentStatus = this.visID === 'Psafe-App-Overview-General-Agents-status';
  ...
  renderComplete = async () => {
    ...
    if (totalTabVis < 1) {
      ...
      } else if (this.visID !== 'Psafe-App-Overview-General-Agents-status') {
  ...
  showDateRangePicker = () => {
    return !this.deadField && !this.state.visRefreshingIndex && this.visID === 'Psafe-App-Overview-General-Agents-status'
  }
