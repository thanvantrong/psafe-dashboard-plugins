...
import { PSAFE_ALERTS_PATTERN } from '../../common/constants';
import {
...
const psafeApp = getAngularModule();
...
function discoverController(
  ...
  //PSAFE
  psafeApp.discoverScope = $scope;
  if (!psafeApp.globalFilters) {
    psafeApp.globalFilters = {};
  }
        ...
        subscribeWithScope(
          ...
          searchBarChanges,
          {
            next: () => {
              ...
              // Psafe. Hides the alerts of the '000' agent if it is in the configuration
              const buildFilters = () => {
                const { hideManagerAlerts } = psafeConfig.getConfig();
                  ...
                  return [
                    {
                      meta: {
                        ...
                        params: { query: '000' },
                        type: 'phrase',
                        index: AppState.getCurrentPattern() || PSAFE_ALERTS_PATTERN
                      },
              ...
              ///////////////////////////////  PSAFE   ///////////////////////////////////
              if (psafeApp.globalFilters && psafeApp.globalFilters.tab === $location.search().tab) {
                psafeApp.globalFilters = {
        ...
        (function updateResultState() {
          ...
          function pick(rows, oldRows, fetchStatus) {
            ...
            if (rowsEmpty && fetchStatus === fetchStatuses.LOADING) return status.LOADING;
              ...
              // Psafe. If there are hits but no rows, the it's also a READY status
          ...
          return function () {
            ...
            // Copying it to the rootScope to access it from the Psafe App //
  ...
  ////////////////////////////////////////////////////////////////////////
  // Psafe - Removed saveDataSource, it's not needed by our integration //
  ////////////////////////////////////////////////////////////////////////
  ...
  $scope.handleRefresh = function ({ query }, isUpdate = true) {
    // Psafe filters are not ready yet
    if (!filtersAreReady()) return;
  ...
  $scope.updateDataSource = () => {
    ...
    // Psafe. Should we fetch "_source" and "hits" ?
    const noHits = isPanels || isClusterMonitoring;

    // Psafe. The very first time, the copies are null, just create them
    ...
    // Psafe. Select the proper searchSource depending on the view
    $scope.searchSource = noHits ? noHitsSearchSource : defaultSearchSource;

    // Psafe. Set the size to 0 depending on the selected searchSource
    const size = noHits ? 0 : $scope.opts.sampleSize;
    searchSource
      .setField('index', $scope.indexPattern)
      .setField('size', size) // Psafe. Use custom size
      .setField(
        'sort',
        getSortForSearchSource(
          $scope.state.sort,
    ...
    //Psafe update the visualizations
    $rootScope.$broadcast('updateVis');
    store.dispatch(updateVis({ update: true }));
  ...
  async function setupVisualization() {
    // Psafe. Do not setup visualization if there isn't a copy for the default searchSource
  ...
  $scope.loadFilters = async (paCurrentFilters, tab) => {
    filterManager.removeAll();
    const appState = appStateContainer.getState();
    if (!appState) {
      $timeout(100).then(() => {
        return loadFilters(paCurrentFilters);
      });
    } else {
      paCurrentFilters.forEach(x => {
        if (x.$state.isImplicit != false)
          x.$state.isImplicit = true
      });
      psafeApp.globalFilters.tab = tab;
      const globalFilters = psafeApp.globalFilters.filters || [];
      if (tab && $scope.tab !== tab) {
        filterManager.removeAll();
      }

      filterManager.addFilters([...paCurrentFilters, ...(globalFilters || [])]);
      $scope.filters = filterManager.filters;
    }
  };
  ...
  /**
   * Psafe - aux function for checking filters status
   */
