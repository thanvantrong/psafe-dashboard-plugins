/*
 * Psafe app - Helper class for div heights
...
export class DynamicHeight {
    ...
    const interval = setInterval(() => {
      ...
      if (editorContainer.length) {
        ...
        setTimeout(() => {
          ...
          const headerContainer = $('.paXmlEditorHeader');
  ...
  static dynamicHeightXmlEditor() {
    let editorContainer;
    const interval = setInterval(() => {
      editorContainer = $('.paXmlEditor');
      if (editorContainer.length) {
        clearInterval(interval);
        setTimeout(() => {
          const editorContainer = $('.paXmlEditor');
          const headerContainer = $('#paXmlEditorHeader');
          const windows = $(window).height();
          const offsetTop = DynamicHeight.getPosition(editorContainer[0]).y;
          editorContainer.height(windows - (offsetTop + 20));
          $('.paXmlEditorBody .CodeMirror').css({
  ...
  static dynamicHeightDevTools(self, window) {
    let devToolsElement;
    const interval = setInterval(() => {
      devToolsElement = $('#pa-dev-left-column');
      if (devToolsElement) {
        clearInterval(interval);
        setTimeout(() => {
          const windows = $(window).height();
          $('#pa-dev-left-column').height(
            windows -
              (DynamicHeight.getPosition($('#pa-dev-left-column')[0]).y + 20)
          );
          $('.pa-dev-column-separator').height(
            windows -
              (DynamicHeight.getPosition($('.pa-dev-column-separator')[0]).y +
                20)
          );
          $('#pa-dev-right-column').height(
            windows -
              (DynamicHeight.getPosition($('#pa-dev-right-column')[0]).y + 20)
          );
          $('.pa-dev-column-separator span').height(
            windows -
              (DynamicHeight.getPosition($('.pa-dev-column-separator span')[0])
                .y +
                20)
          );
        }, 1);
