/*
 * Psafe app - DevTools history component
...
export function DevToolsHistory({ localStorage, closeHistory, addRequest }) {
    ...
    const getHistoryFromLocalStorageOrdered = () => {
        const psafeHistoryElems = Object.keys(localStorage).filter(x => x.startsWith('psafe:history.elem_')).flatMap(x => {
            const time = x.split("_")[1];
            return [[x, parseInt(time)]]
        }).sort(function (a, b) { return b[1] - a[1] });
        while(psafeHistoryElems.length > 150){ // we only maintain the last 150 requests
            const lastEl = psafeHistoryElems[psafeHistoryElems.length-1];
            window.localStorage.removeItem(lastEl[0])
            psafeHistoryElems.pop();
        }
        return psafeHistoryElems;
    }
    ...
    const showHistoryList = () => {
        const psafeElements = getHistoryFromLocalStorageOrdered();
        const elements = psafeElements.map(x => {
        ...
        return psafeElements.length ? <ul className="history-list">
            {elements}
        </ul> : <></>
    }
    ...
    const removeHistory = () => {
        Object.keys(localStorage).filter(x => x.startsWith('psafe:history.elem_')).forEach(item => {
