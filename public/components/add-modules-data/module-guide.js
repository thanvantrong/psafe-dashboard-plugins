/*
* Psafe app - React component for show a module guide.
...
class PaModuleGuide extends Component {
  constructor(props) {
    super(props);
  ...
  renderSteps() {
    ...
    return [
      ...configurationSteps,
      {
        title: !invalidConfiguration ? 'Edit optit.conf' : 'Configuration error',
        children: (
          <Fragment>
              ...
              <Fragment>
                ...
                {this.state.agentTypeSelected === 'manager' ? (
                  <Fragment>
                    ...
                    <EuiText>When you finish of configure the module, copy above XML configuration. Go to <EuiCode>/var/optit/etc</EuiCode> in Linux agent, include the above configuration in <EuiCode>optit.conf</EuiCode> and restart the agent.</EuiText>
                    ) : (
                      <EuiText>When you finish of configure the module, copy above XML configuration. Go to <EuiCode>C:\Program Files (x86)\optit-agent</EuiCode> in Windows agent, include the above configuration in <EuiCode>optit.conf</EuiCode> and restart the agent.</EuiText>
                )}
                <EuiSpacer size='s' />
                <EuiText>The above section must be located within the top-level <EuiCode>{`<optit_config>`}</EuiCode> tag.</EuiText>
              </Fragment>
              ) : (
