/*
 * Psafe app - React component for add sample data
...
export default class PaSampleData extends Component {
  categories: {
    title: string;
    description: string;
    image: string;
  ...
  async componentDidMount() {
    try {
      ...
      try {
        const results = await PromiseAllRecursiveObject(
          this.categories.reduce((accum, cur) => {
            accum[cur.categorySampleAlertsIndex] = PaRequest.genericReq(
              'GET',
      ...
      const options = {
        context: `${PaSampleData.name}.componentDidMount`,
  ...
  async addSampleData(category) {
    try {
      ...
      const options = {
        context: `${PaSampleData.name}.addSampleData`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
  ...
  async removeSampleData(category) {
    try {
      ...
      const options = {
        context: `${PaSampleData.name}.removeSampleData`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
            ...
            <EuiFlexGroup justifyContent="flexEnd">
              <EuiFlexItem grow={false}>
                {(exists && (
                  <PaButtonPermissions
                    color="danger"
                    roles={[PSAFE_ROLE_ADMINISTRATOR_NAME]}
