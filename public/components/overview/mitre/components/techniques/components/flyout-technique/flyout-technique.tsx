/*
 * Psafe app - Mitre flyout components
...
export class FlyoutTechnique extends Component {
  ...
  async getTechniqueData() {
    try {
      ...
      const { currentTechnique } = this.props;
      const techniqueResponse = await PaRequest.apiReq('GET', '/mitre/techniques', {
        params: {
          q: `external_id=${currentTechnique}`,
        },
      });
      const [techniqueData] = (((techniqueResponse || {}).data || {}).data || {}).affected_items;
      const tacticsResponse = await PaRequest.apiReq('GET', '/mitre/tactics', {});
  ...
  renderBody() {
    ...
    const formattedDescription = techniqueData.description ? (
      <div
        className="pa-markdown-margin pa-markdown-wrapper"
  ...
  render() {
    ...
    return (
      <PaFlyout
        ...
        flyoutProps={{
          size: 'l',
          className: 'flyout-no-overlap pa-inventory paApp',
