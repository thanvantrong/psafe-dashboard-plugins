/*
 * Psafe app - Mitre alerts components
...
export const Techniques = withWindowSize(
  class Techniques extends Component {
    ...
    async getMitreTechniques(params) {
      try {
        return await PaRequest.apiReq('GET', '/mitre/techniques', { params });
      } catch (error) {
        const options = {
      ...
      const tacticsToRenderOrdered = tacticsToRender
        ...
        .map((item, idx) => {
          ...
          const toolTipAnchorClass =
            'pa-display-inline-grid' + (this.state.hover === item.id ? ' pa-mitre-width' : ' ');
          return (
            <EuiFlexItem
    ...
    onSearch = async (searchValue) => {
      try {
        if (searchValue) {
          ...
          const response = await PaRequest.apiReq('GET', '/mitre/techniques', {
            params: {
              search: searchValue,
            },
          });
