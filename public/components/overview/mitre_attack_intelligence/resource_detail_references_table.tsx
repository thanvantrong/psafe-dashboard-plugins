/*
 * Psafe app - React component for showing the Mitre Att&ck intelligence flyout tables.
 *
  ...
  const getValues = async () => {
    ...
    try{
      const data = await Promise.all(namesConcatenated.map(async (nameConcatenated) => {
        const queryResult = await PaRequest.apiReq('GET', `/mitre/${referencesName}?${referencesName.replace(/s\s*$/, '')}_ids=${nameConcatenated}`, {});
