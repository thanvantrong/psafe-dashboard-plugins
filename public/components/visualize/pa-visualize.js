/*
 * Psafe app - React component for Visualize.
...
export const PaVisualize = compose(
  withErrorBoundary,
  withReduxProvider
)(
  class PaVisualize extends Component {
    _isMount = false;
    constructor(props) {
      super(props);
      this.state = {
      ...
      this.monitoringEnabled = !!(configuration || {})['psafe.monitoring.enabled'];
      this.newFields = {};
    }
     ...
     * 
     * More info:
     * https://github.com/psafe/psafe-kibana-app/issues/4230#issuecomment-1152161434
     * 
    ...
    refreshKnownFields = async (newField = null) => {
      ...
      if (!this.hasRefreshedKnownFields) {
        ...
        try {
          ...
          this.isRefreshing = false;
          const options = {
            context: `${PaVisualize.name}.refreshKnownFields`,
        ...
        getToasts().add({
          ...
          text: toMountPoint(<EuiFlexGroup justifyContent="flexEnd" gutterSize="s">
            <EuiFlexItem grow={false}>
              ...
              <a
                title="More information in Psafe documentation"
            ...
            >
              <EuiFlexItem className="h-100">
                <EuiFlexGroup style={{ padding: '12px 12px 0px' }} className="embPanel__header">
                  <h2 className="embPanel__title pa-headline-title">{vis.title}</h2>
                  <EuiButtonIcon
                    color="text"
      ...
      return (
        <Fragment>
          ...
          {this.props.resultState === 'none' && (
            <div className="pa-margin-top-10 pa-margin-right-8 pa-margin-left-8">
              <EuiCallOut
                ...
                <EuiPanel
                  ...
                  className={
                    this.state.expandedVis === 'security-alerts'
                      ? 'fullscreen h-100 pa-overflow-y-auto pa-overflow-x-hidden'
                      : 'h-100'
                  }
                >
                  <EuiFlexItem className="h-100" style={{ marginBottom: 12 }}>
                    <EuiFlexGroup style={{ padding: '12px 12px 0px' }} className="embPanel__header">
                      <h2 className="embPanel__title pa-headline-title">Security Alerts</h2>
                      <EuiButtonIcon
                        color="text"
                        style={{ padding: '0px 6px', height: 30 }}
