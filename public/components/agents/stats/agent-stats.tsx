/*
 * Psafe app - Component to display the Agent stats
...
import { PaRequest, formatUIDate } from '../../../react-services';
...
export const MainAgentStats = compose(
  ...
  withGuard(({agent}) => {
    const [major, minor, patch] = agent.version.replace('Psafe v','').split('.').map(value => parseInt(value));
...
function AgentStats({agent}){
  ...
  useEffect(() => {
    (async function(){
      setLoading(true);
      try{
        const responseDataStatLogcollector = await PaRequest.apiReq('GET', `/agents/${agent.id}/stats/logcollector`, {});
        const responseDataStatAgent = await PaRequest.apiReq('GET', `/agents/${agent.id}/stats/agent`, {});
