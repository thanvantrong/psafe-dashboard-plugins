...
export class Inventory extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      if (match && match[0]) {
        ...
        const policy = await PaRequest.apiReq('GET', `/sca/${this.props.agent.id}`, {
          q: 'policy_id=' + id,
        });
  ...
  async initialize() {
    try {
      ...
      } = await PaRequest.apiReq('GET', `/sca/${this.props.agent.id}`, {});
  ...
  async loadScaPolicy(policy) {
    ...
    if (policy) {
      try {
        const policyResponse = await PaRequest.apiReq('GET', `/sca/${this.props.agent.id}`, {
          params: {
        ...
        const checksResponse = await PaRequest.apiReq(
          'GET',
