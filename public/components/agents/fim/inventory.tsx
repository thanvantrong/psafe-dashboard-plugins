...
import { filtersToObject } from '../../pa-search-bar';
...
export class Inventory extends Component {
  ...
  getStoreFilters(props) {
    ...
    const filters = JSON.parse(window.localStorage.getItem(`psafe-${section}-${selectView}-${((this.state || {}).selectedTabId || 'files')}-${agent['id']}`) || '{}');
    return filters;
  }

  setStoreFilters(filters) {
    const { section, selectView, agent } = this.props;
    window.localStorage.setItem(`psafe-${section}-${selectView}-${(this.state || {}).selectedTabId || 'files'}-${agent['id']}`, JSON.stringify(filters))
  }
  ...
  async getItemNumber(type: 'file' | 'registry') {
    try {
      ...
      const response = await PaRequest.apiReq('GET', `/syscheck/${agentID}`, {
