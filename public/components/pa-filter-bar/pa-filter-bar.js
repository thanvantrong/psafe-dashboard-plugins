...
import './pa-filter-bar.scss';
import { withErrorBoundary } from '../common/hocs';
import { UI_LOGGER_LEVELS } from '../../../common/constants';
import { UI_ERROR_SEVERITIES } from '../../react-services/error-orchestrator/types';
import { getErrorOrchestrator } from '../../react-services/common-services';

export const PaFilterBar = withErrorBoundary(
  class PaFilterBar extends Component {
    constructor(props) {
      super(props);
      ...
      if (this.state.isProcessing) {
        for (const i in this.state.selectedOptions) {
          if (this.state.selectedOptions.hasOwnProperty(i)) {
            ...
            const el = $('.paFilterBarOperator .euiBadge__content')[i];
            const hasBtn = $(el).find('.paFilterBarOperatorBtn');
            if (hasBtn.length) {
              $(hasBtn[0]).remove();
            }
            if (i != 0) {
              if (selectedOptions.type != 'search') {
                const button = $(
                  `<button class="paFilterBarOperatorBtn euiButtonEmpty euiButtonEmpty--primary euiButtonEmpty--xSmall"><b>${selectedOptions.type}<b></button>`
                );
                ...
                const button = $(`<span class="paFilterBarOperatorBtn"><b>AND<b></button>`);
                $(el).prepend(button);
              }
            }
          }
    ...
    encodeFilter(option) {
      ...
      newFilter.className = 'paFilterBarOperator';
    }
    ...
    buildQuery = () => {
      try {
        ...
        const options = {
          context: `${PaFilterBar.name}.buildQuery`,
