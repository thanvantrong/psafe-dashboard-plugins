...
export class MainModuleAgent extends Component {
  ...
  setGlobalBreadcrumb() {
    ...
    if (this.props.section === 'welcome') {
      ...
      breadcrumb = [
        ...
        {
          text: PSAFE_MODULES[this.props.section].title,
          className: 'pa-global-breadcrumb-popover'
        },
      ];
    }
  ...
  showAgentInfo() {
    const elem = document.getElementsByClassName('pa-module-body-main')[0];
    if (elem) {
      if (!this.state.showAgentInfo) {
        elem.classList.add("pa-module-body-main-double");
      } else {
        elem.classList.remove("pa-module-body-main-double");
      }
    }
          ...
          <EuiFlexGroup>
            <EuiFlexItem grow={false}>
              <span style={{ display: 'inline-flex' }}>
                <EuiTitle size="s" className="pa-module-header-agent-title-btn">
                  <h1>
                    <span
                      onClick={() => {
            ...
            {/*             <EuiFlexItem grow={false} style={{ margin: '16px 0' }} className="pa-module-header-agent-title-btn">
  ...
  render() {
    ...
    return (
      <div className={this.state.showAgentInfo ? 'pa-module pa-module-showing-agent' : 'pa-module'}>
        <div className='pa-module-header-agent-wrapper'>
          <div className='pa-module-header-agent'>
            {title}
          </div>
        </div>
        {(agent && agent.os) &&
          <Fragment>
            <div className='pa-module-header-nav-wrapper'>
              <div className={this.props.tabs && this.props.tabs.length && 'pa-module-header-nav'}>
                {this.state.showAgentInfo &&
                  <div className={
                    !this.props.tabs || !this.props.tabs.length ?
                      "pa-welcome-page-agent-info" :
                      "pa-welcome-page-agent-info pa-welcome-page-agent-info-gray"}>
                    <AgentInfo agent={this.props.agent} isCondensed={false} hideActions={true} {...this.props}></AgentInfo>
                  </div>
                }
                {(this.props.tabs && this.props.tabs.length) &&
                  <div className="pa-welcome-page-agent-tabs">
                    <EuiFlexGroup>
