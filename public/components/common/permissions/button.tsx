/*
 * Psafe app - Button with Psafe API permissions and/or roles required to be useful
...
import { PaPermissionsFormatted } from './format';
...
interface IPaButtonPermissionsProps{
...
export const PaButtonPermissions = ({permissions = null, roles = null, buttonType = 'default', tooltip, ...rest} : IPaButtonPermissionsProps) => {
      ...
      {userPermissionRequirements && (
        <div>
          <div>Require the {userPermissionRequirements.length === 1 ? 'permission' : 'permissions'}:</div>
            {PaPermissionsFormatted(userPermissionRequirements)}
        </div>
      )}
