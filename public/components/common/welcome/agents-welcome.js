...
import PaTextWithTooltipIfTruncated from '../pa-text-with-tooltip-if-truncated';
...
class AgentsWelcome extends Component {
  ...
  renderTitle() {
    ...
    return (
      <EuiFlexGroup>
        <EuiFlexItem className="pa-module-header-agent-title">
          <EuiFlexGroup>
            <EuiFlexItem grow={false}>
              <span style={{ display: 'inline-flex' }}>
                <EuiTitle size="s">
                  <h1>
                    <PaTextWithTooltipIfTruncated position='top' elementStyle={{ maxWidth: "150px" }}>
    ...
    return (
      <EuiFlexItem>
        <EuiCard
          ...
          title={PSAFE_MODULES[tab].title}
          onClick={() => this.props.switchTab(tab)}
          description={PSAFE_MODULES[tab].description}
        />
      </EuiFlexItem>
    );
  }
  ...
  getRequirementVis() {
    if (this.state.selectedRequirement === 'pci') {
      return 'Psafe-App-Agents-Welcome-Top-PCI';
    }
    if (this.state.selectedRequirement === 'gdpr') {
      return 'Psafe-App-Agents-Welcome-Top-GDPR';
    }
    if (this.state.selectedRequirement === 'hipaa') {
      return 'Psafe-App-Agents-Welcome-Top-HIPAA';
    }
    if (this.state.selectedRequirement === 'nist') {
      return 'Psafe-App-Agents-Welcome-Top-NIST-800-53';
    }
    if (this.state.selectedRequirement === 'gpg13') {
      return 'Psafe-App-Agents-Welcome-Top-GPG-13';
    }
    if (this.state.selectedRequirement === 'tsc') {
      return 'Psafe-App-Agents-Welcome-Top-TSC';
    }
    return 'Psafe-App-Agents-Welcome-Top-PCI'
  }
      ...
      <Fragment>
        <EuiPanel paddingSize="s" height={{ height: 300 }}>
          <EuiFlexItem>
            <EuiFlexGroup>
              <EuiFlexItem>
                <h2 className="embPanel__title pa-headline-title">
      ...
      <EuiPanel paddingSize="s" >
        <EuiFlexItem>
          ...
          <div style={{ height: this.props.resultState !== 'loading' ? '225px' : 0 }}>
            <PaReduxProvider>
              <KibanaVis
                visID={'Psafe-App-Agents-Welcome-Events-Evolution'}
                tab={'welcome'}
              ></KibanaVis>
            </PaReduxProvider>
          </div>
  ...
  render() {
          ...
          body={
            <Fragment>
              ...
              <a href={webDocumentationLink('user-manual/agents/agent-connection.html')} target="_blank">
                Checking connection with the Psafe server
              </a>
            </Fragment>
          }
          actions={
    ...
    return (
      <div className="pa-module pa-module-welcome">
        <div className='pa-module-header-agent-wrapper'>
          <div className='pa-module-header-agent pa-module-header-agent-main'>
            {title}
          </div>
        </div>
        <div>
          <div className='pa-module-header-nav-wrapper'>
            <div className='pa-module-header-nav'>
              <div style={{ margin: '0 16px' }}>
                <EuiPanel grow paddingSize='s' className="pa-welcome-page-agent-info">
        ...
        <div className='pa-module-body pa-module-agents-padding-responsive'>
          <EuiPage>
            <EuiPageBody component="div">
              <EuiFlexGroup>
                <EuiFlexItem />
                <EuiFlexItem style={{ alignItems: 'flex-end', marginTop: 6, marginBottom: 10 }}> {/* DatePicker */}
                  <PaDatePicker condensed={true} onTimeChange={() => { }} />
                </EuiFlexItem>
              </EuiFlexGroup>
              {this.state.widthWindow < 1150 && (
                <Fragment>
                  <EuiFlexGrid columns={2}>
                    <EuiFlexItem key={'Psafe-App-Agents-Welcome-MITRE-Top-Tactics'} >
                  ...
                  <EuiSpacer size='m' />
                  <EuiFlexGroup>
                    <EuiFlexItem key={'Psafe-App-Agents-Welcome-Events-Evolution'} > {/* Events count evolution */}
