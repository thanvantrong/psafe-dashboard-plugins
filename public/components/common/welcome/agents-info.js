/*
 * Psafe app - React component for showing agent fields such as IP, ID, name,
...
export class AgentInfo extends Component {
  ...
  addTextPlatformRender(agent, style) {
    ...
    return (
      <PaTextWithTooltipIfTruncated position='bottom' tooltipProps={{ anchorClassName: 'pa-width-100'}} elementStyle={{ maxWidth: style.maxWidth, fontSize: 12 }}>
  ...
  buildStats(items) {    
    ...
    const stats = items.map(item => {
      ...
      const tooltipProps = item.description === 'Cluster node' ? { anchorClassName: 'pa-width-100'} : {};
      return (
          ...
          <PaStat
            title={
              item.description === 'Groups' ? (
                ...
                <PaTextWithTooltipIfTruncated position='bottom' tooltipProps={tooltipProps} elementStyle={{ maxWidth: item.style.maxWidth, fontSize: 12 }}>
  ...
  render() {
    ...
    return (
      <Fragment>
        <EuiFlexGroup wrap style={{responsive: true }} className="pa-welcome-page-agent-info-details">
          {stats}
        </EuiFlexGroup>
      </Fragment>
    );
