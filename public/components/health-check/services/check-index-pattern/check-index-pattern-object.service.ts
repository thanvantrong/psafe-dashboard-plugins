/*
 * Psafe app - Check alerts index pattern service
 *
...
export const checkIndexPatternObjectService =  async (appConfig, checkLogger: CheckLogger) => {
  ...
  let listValidIndexPatterns = await SavedObject.getListOfPsafeValidIndexPatterns(defaultIndexPatterns, HEALTH_CHECK);
  ...
  if (!indexPatternDefaultFound && defaultPatternId) {
    // if no valid index patterns are found we try to create the psafe-alerts-*
    try {
      ...
      if (existDefaultIndexPattern) {
        ...
        await SavedObject.createPsafeIndexPattern(defaultPatternId);
      ...
      checkLogger.info(`Getting list of valid index patterns...`);
      listValidIndexPatterns = await SavedObject.getListOfPsafeValidIndexPatterns(defaultIndexPatterns, HEALTH_CHECK);
      checkLogger.info(`Valid index patterns found: ${listValidIndexPatterns.length || 0}`);
      if(!AppState.getCurrentPattern()){
        // Check the index pattern saved objects can be found using `GET /api/saved_objects/_find` endpoint.
        // Related issue: https://github.com/psafe/psafe-kibana-app/issues/4293
