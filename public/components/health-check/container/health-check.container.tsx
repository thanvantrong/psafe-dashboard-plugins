/*
 * Psafe app - Health Check Component
 *
...
import {
  ...
  PSAFE_INDEX_TYPE_MONITORING,
  PSAFE_INDEX_TYPE_STATISTICS,
  PSAFE_PLUGIN_PLATFORM_SETTING_MAX_BUCKETS,
  PSAFE_PLUGIN_PLATFORM_SETTING_METAFIELDS,
  PSAFE_PLUGIN_PLATFORM_SETTING_TIME_FILTER,
...
const checks = {
  api: {
    title: 'Check Psafe API connection',
  ...
  patternMonitoring: {
    ...
    validator: (appConfig) => checkPatternSupportService(appConfig.data['psafe.monitoring.pattern'], PSAFE_INDEX_TYPE_MONITORING),
    awaitFor: [],
    shouldCheck: true,
    canRetry: true,
  },
  patternStatistics: {
    ...
    validator: (appConfig) => checkPatternSupportService(`${appConfig.data['cron.prefix']}-${appConfig.data['cron.statistics.index.name']}-*`, PSAFE_INDEX_TYPE_STATISTICS),
    awaitFor: [],
    shouldCheck: true,
    canRetry: true,
  },
  maxBuckets: {
    ...
    validator: checkPluginPlatformSettings(PLUGIN_PLATFORM_SETTING_NAME_MAX_BUCKETS, PSAFE_PLUGIN_PLATFORM_SETTING_MAX_BUCKETS),
    awaitFor: [],
    canRetry: true,
  },
  metaFields: {
    ...
    validator: checkPluginPlatformSettings(PLUGIN_PLATFORM_SETTING_NAME_METAFIELDS, PSAFE_PLUGIN_PLATFORM_SETTING_METAFIELDS),
    awaitFor: [],
    canRetry: true,
  },
  timeFilter: {
    ...
    validator: checkPluginPlatformSettings(PLUGIN_PLATFORM_SETTING_NAME_TIME_FILTER, JSON.stringify(PSAFE_PLUGIN_PLATFORM_SETTING_TIME_FILTER), (checkLogger: CheckLogger, options: {defaultAppValue: any}) => {
      getDataPlugin().query.timefilter.timefilter.setTime(PSAFE_PLUGIN_PLATFORM_SETTING_TIME_FILTER)
...
function HealthCheckComponent() {
  ...
  const redirectionPassHealthcheck = () => {
    ...
    const url = '/app/psafe#' + ($rootScope.previousLocation || '') + '?' + queryString;
          ...
          <EuiFlexGroup justifyContent='center'>
            {thereAreErrors && (
              <EuiFlexItem grow={false}>
                <EuiButton fill href={getHttp().basePath.prepend('/app/psafe#/settings')}>
                  Go to Settings
                </EuiButton>
              </EuiFlexItem>
            )}
