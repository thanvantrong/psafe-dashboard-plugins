/*
 * Psafe app - React component for the adding an API entry form.
 *
...
export const ApiIsDown = withErrorBoundary (class ApiIsDown extends Component {
      ...
      for (let idx in entries) {
        ...
        try {
          ...
          const downReason = typeof error === 'string' ? error :
            (error || {}).message || ((error || {}).data || {}).message || 'Psafe is not reachable';
  ...
  render() {
    const apiExample = `# Example Psafe API configuration
hosts:
    - production:
...
`;
    ...
    const checkConnectionChildren = (
      <div>
        <EuiText>
          Check that the {PLUGIN_PLATFORM_NAME} server can reach the configured Psafe API(s).
        </EuiText>
        <EuiSpacer />
        <EuiButton
        ...
        <EuiSpacer />
        <EuiText>Already configured Psafe API(s)</EuiText>
        <EuiSpacer />
        {(!this.state.error && (
          <EuiBasicTable
    ...
    const steps = [
      {
        title: 'Check the Psafe API service status',
        children: (
          <div>
            <EuiText>For Systemd</EuiText>
            <EuiSpacer />
            <EuiCode>$ sudo systemctl status psafe-manager</EuiCode>
            <EuiSpacer />
            <EuiText>For SysV Init</EuiText>
            <EuiSpacer />
            <EuiCode>$ sudo service psafe-manager status</EuiCode>
          </div>
        )
      },
      {
        ...
        children: (
          <div>
            <EuiText>
              ...
              <EuiCode>
                {getPluginDataPath('config/psafe.yml')}
              </EuiCode>{' '}
              file.
            </EuiText>
            <EuiSpacer />
    ...
    return (
      <EuiFlexGroup>
        ...
        <EuiFlexItem className="min-guide-width">
          <EuiPanel>
            <EuiText>
              <h2>Psafe API seems to be down</h2>
            </EuiText>
            <EuiSpacer />
