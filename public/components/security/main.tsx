...
import { PSAFE_ROLE_ADMINISTRATOR_NAME, PLUGIN_PLATFORM_NAME } from '../../../common/constants';
...
export const PaSecurity = compose(
  withErrorBoundary,
  withReduxProvider,
  ...
  useEffect(() => {
    try {
      ...
      const options = {
        context: `${PaSecurity.name}.fetchAllowRunAs`,
  ...
  const isNotRunAs = (allowRunAs) => {
    ...
    switch (allowRunAs) {
      case API_USER_STATUS_RUN_AS.HOST_DISABLED:
        runAsWarningTxt =
          `For the role mapping to take effect, enable run_as in ${getPluginDataPath('config/psafe.yml')} configuration file, restart the ${PLUGIN_PLATFORM_NAME} service and clear your browser cache and cookies.`;
        break;
      case API_USER_STATUS_RUN_AS.USER_NOT_ALLOWED:
        runAsWarningTxt =
          'The role mapping has no effect because the current Psafe API user has allow_run_as disabled.';
        break;
      case API_USER_STATUS_RUN_AS.ALL_DISABLED:
        runAsWarningTxt =
          `For the role mapping to take effect, enable run_as in ${getPluginDataPath('config/psafe.yml')} configuration file and set the current Psafe API user allow_run_as to true. Restart the ${PLUGIN_PLATFORM_NAME} service and clear your browser cache and cookies.`;
        break;
      default:
        runAsWarningTxt =
          'The role mapping has no effect because the current Psafe API user has run_as disabled.';
        break;
    }
