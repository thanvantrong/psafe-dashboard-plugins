...
import { PaFlyout } from '../../common/flyouts';

export const EditPolicyFlyout = ({ policy, closeFlyout }) => {
  const isReserved = PaAPIUtils.isReservedID(policy.id);
  ...
  const updatePolicy = async () => {
    try {
      ...
      const response = await PaRequest.apiReq('PUT', `/security/policies/${policy.id}`, {
        policy: {
          actions: actions,
          resources: resources,
          effect: effectValue,
  ...
  async function getData() {
    const resources_request = await PaRequest.apiReq('GET', '/security/resources', {});
    const actions_request = await PaRequest.apiReq('GET', '/security/actions', {});
  ...
  return (
    <>
      <PaFlyout flyoutProps={{ className: 'paApp' }} onClose={onClose}>
