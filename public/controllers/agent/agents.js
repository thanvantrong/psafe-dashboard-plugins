...
import { API_NAME_AGENT_STATUS, PSAFE_ALERTS_PATTERN } from '../../../common/constants';
...
export class AgentsController {
  ...
  async $onInit() {
    ...
    this.$scope.TabDescription = PSAFE_MODULES;
    ...
    this.$scope.search = (term, specificPath) =>
      this.$scope.$broadcast('psafeSearch', {
        term,
        specificPath,
      });
    ...
    this.$scope.restartAgent = async (agent) => {
      ...
      try {
        const data = await PaRequest.apiReq('PUT', `/agents/${agent.id}/restart`, {});
    ...
    this.$scope.confirmAddGroup = async (group) => {
      try {
        ...
        const response = await PaRequest.apiReq('GET', `/agents`, {
          params: {
  ...
  async switchTab(tab, force = false) {
    ...
    if (!force && ((this.$scope || {}).agent || false)) {
      try {
        const agentInfo = await PaRequest.apiReq('GET', `/agents`, {
          params: {
            agents_list: this.$scope.agent.id,
            select: 'status',
          },
  ...
  async onClickUpgrade() {
    try {
      await PaRequest.apiReq('PUT', `/agents/${this.$scope.agent.id}/upgrade`, {});
  ...
  async onClickRestart() {
    try {
      await PaRequest.apiReq('PUT', `/agents/${this.$scope.agent.id}/restart`, {});
  ...
  async getAgent(newAgentId) {
    try {
      ...
      const data = await PaRequest.apiReq('GET', `/agents`, {
        params: {
          agents_list: id,
        },
      });
      ...
      const groups = await PaRequest.apiReq('GET', '/groups', {});
      ...
      if (!this.$scope.agent) {
        if ((error || {}).status === -1) {
          this.$scope.emptyAgent = 'Psafe API timeout.';
        }
      }
  ...
  async launchRootcheckScan() {
    try {
      ...
      await PaRequest.apiReq('PUT', `/rootcheck/${this.$scope.agent.id}`, {});
      ErrorHandler.info(
  ...
  async launchSyscheckScan() {
    try {
      ...
      await PaRequest.apiReq('PUT', `/syscheck`, {
        params: {
          agents_list: this.$scope.agent.id,
        },
      });
