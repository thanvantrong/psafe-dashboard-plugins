/*
 * Psafe app - Agents preview controller
...
export class AgentsPreviewController {
  ...
  async $onInit() {
    ...
    const summaryData = await PaRequest.apiReq('GET', '/agents/summary/status', {});
    ...
    this.$scope.$on('psafeFetched', (evt) => {
      evt.stopPropagation();
    });
    this.registerAgentsProps = {
      ...
      getPsafeVersion: () => this.getPsafeVersion(),
      getCurrentApiAddress: () => this.getCurrentApiAddress()
    };
    this.hasAgents = true;
    this.init = false;
    const instance = new DataFactory(PaRequest.apiReq, '/agents', false, false);
    //Props
    this.tableAgentsProps = {
      paReq: (method, path, body) => PaRequest.apiReq(method, path, body),
      addingNewAgent: () => {
        this.addNewAgent(true);
   ...
   */
  query(query, search) {
    this.$scope.$broadcast('psafeQuery', { query, search });
    ...
    try {
      ...
      const options = {
        ...
        error: {
          error: error,
          message: error.message || error,
          title: `Could not get the Psafe API address: ${error.message || error}`,
        },
      };
      getErrorOrchestrator().handleError(options);
    }
  ...
  /**
   * Returns the Psafe version as x.y.z
   */
  async getPsafeVersion() {
    try {
      const data = await PaRequest.apiReq('GET', '/', {});
      const result = ((data || {}).data || {}).data || {};
      return result.api_version;
    } catch (error) {
      const options = {
        context: `${AgentsPreviewController.name}.getPsafeVersion`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
          message: error.message || error,
          title: `Could not get the Psafe version: ${error.message || error}`,
        },
      };
      getErrorOrchestrator().handleError(options);
      return version;
