/*
 * Psafe app - React component for building the agents table.
 *
...
import { PaSearchBar, filtersToObject } from '../../../components/pa-search-bar';
...
export const AgentsTable = withErrorBoundary(
  class AgentsTable extends Component {
    ...
    async UNSAFE_componentWillMount() {
      const managerVersion = await PaRequest.apiReq('GET', '/', {});
      const totalAgent = await PaRequest.apiReq('GET', '/agents', {});
      const agentActive = await PaRequest.apiReq('GET', '/agents/summary/status', {});
    ...
    async getItems() {
      try {
        ...
        const rawAgents = await this.props.paReq('GET', '/agents', { params: this.buildFilter() });
    ...
    async getAllItems() {
      ...
      const rawAgents = await this.props.paReq('GET', '/agents', filterTable);

      const agentsFiltered = await this.props.paReq('GET', '/agents', filterAll).then(() => {
    ...
    renderUpgradeButtonAll() {
      ...
      if (
        ...
        avaibleAgents.filter(
          (agent) => agent.version !== 'Psafe ' + managerVersion && agent.status === API_NAME_AGENT_STATUS.ACTIVE
        ).length === 0
      ) {
        return;
      }
    ...
    onClickPurge = async () => {
      ...
      try {
        const response = await PaRequest.apiReq('DELETE', `/agents`, {
          purge: true,
          ids: auxAgents,
          older_than: '1s',
        });
    ...
    onClickPurgeAll = () => {
      ...
      const auxAgents = avaibleAgents
        ...
        .filter((agent) => agent !== null);
      try {
        const response = PaRequest.apiReq('DELETE', `/agents`, {
          purge: true,
          ids: auxAgents,
          older_than: '1s',
        });
