...
export class SettingsController {
  ...
  constructor($scope, $window, $location, errorHandler) {
    ...
    this.paMisc = new PaMisc();
    this.psafeConfig = new PsafeConfig();

    if (this.paMisc.getWizard()) {
      $window.sessionStorage.removeItem('healthCheck');
      this.paMisc.setWizard(false);
    }

    this.apiIsDown = this.paMisc.getApiIsDown();
  ...
  async getSettings() {
    try {
      try{
        this.indexPatterns = await SavedObject.getListOfPsafeValidIndexPatterns();
      }catch(error){
        this.paMisc.setBlankScr('Sorry but no valid index patterns were found');
  ...
  async checkManager(item, isIndex, silent = false) {
    try {
      ...
      this.paMisc.setApiIsDown(false);
      ...
      return [
        {
          date: new Date(),
          level: 'error',
          message: 'Error when loading Psafe app logs'
        }
      ];
    }
  }
  ...
  async getAppInfo() {
    try {
      ...
      this.load = false;
      const config = this.psafeConfig.getConfig();
  ...
  /**
   * Checks if there are new APIs entries in the psafe.yml
   */
  async checkForNewApis() {
    try {
      ...
      //Tries to check if there are new APIs entries in the psafe.yml also, checks if some of them have connection
      if (!hosts.length)
        throw {
          message: 'There were not found any API entry in the psafe.yml',
          type: 'warning',
          closedEnabled: false
        };
      ...
      if (notRecheable) {
        if (notRecheable >= hosts.length) {
          ...
          throw {
            message:
              'Psafe API not recheable, please review your configuration',
            type: 'danger',
            closedEnabled: true
          };
        }
        throw {
          message: `Some of the API entries are not reachable. You can still use the Psafe APP but please, review your hosts configuration.`,
          type: 'warning',
          closedEnabled: true
        };
      }
  ...
  /**
   * Get the hosts in the psafe.yml
   */
  async getHosts() {
    try {
