...
import { filtersToObject, PaSearchBar } from '../../../../components/pa-search-bar';
...
export class AgentSelectionTable extends Component {
  ...
  async getItems() {
    try {
      ...
      const rawData = await PaRequest.apiReq('GET', '/agents', { params: this.buildFilter() });
  ...
  async selectAgentAndApply(agentID){
    try{
      const data = await PaRequest.apiReq('GET', '/agents', { params: { q: 'id=' + agentID}});
