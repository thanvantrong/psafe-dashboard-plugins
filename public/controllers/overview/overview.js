...
import { UI_LOGGER_LEVELS, PSAFE_ALERTS_PATTERN } from '../../../common/constants';
...
export class OverviewController {
  ...
  async $onInit() {
    ...
    this.TabDescription = PSAFE_MODULES;
    ...
    this.paMonitoringEnabled = false;
    ...
    if (agent && store.getState().appStateReducers.currentAgentData.id !== agent) {
      ...
      const data = await this.paReq('GET', '/agents', { params });
  ...
  async getSummary() {
    try {
      const {data: { data }} = await PaRequest.apiReq('GET', '/agents/summary/status', {});
  ...
  async loadConfiguration() {
    try {
      const configuration = this.psafeConfig.getConfig();

      this.paMonitoringEnabled = !!configuration['psafe.monitoring.enabled'];

    } catch (error) {
      this.paMonitoringEnabled = true;
      return Promise.reject(error);
    }
  }
