/*
 * Psafe app - Dev tools controller
...
export class DevToolsController {
      ...
      if (item.requestTextJson) {
        try {
          ...
          icon.onmouseover = () => {
            ...
            advice.className = 'lint-block-pa';
          };
  ...
  init() {
    ...
    this.$window.onresize = () => {
      $('#pa-dev-left-column').attr(
        'style',
        'width: calc(30% - 7px); !important'
      );
      $('#pa-dev-right-column').attr(
        'style',
  ...
  calculateWhichGroup(firstTime) {
    try {
      ...
      try {
        ...
        $('#play_button').hide();
        $('#psafe_dev_tools_documentation').hide();
        return null;
      }

      if (!$('#play_button').is(':visible')) $('#play_button').show();
      if (!$('#psafe_dev_tools_documentation').is(':visible')) $('#psafe_dev_tools_documentation').show();
      const currentPlayButton = $('#play_button').offset();
      $('#play_button').offset({
        top: cords.top + 2,
        left: currentPlayButton.left
      });
      $('#psafe_dev_tools_documentation').offset({
        top: cords.top + 2
      });
      ...
      if (desiredGroup[0]) {
        ...
        if (apiEndpoint && apiEndpoint.documentation) {
          $('#psafe_dev_tools_documentation').attr('href', apiEndpoint.documentation).show();
        } else {
          $('#psafe_dev_tools_documentation').attr('href', '').hide();
        }
      }
  ...
  async send(firstTime) {
    try {
      ...
      if (desiredGroup) {
        ...
        if (!firstTime) {
          const output = await this.paRequest.apiReq(method, path, JSONraw);
  ...
  parseError(error){
    if ((error || {}).status === -1) {
      return 'Psafe API is not reachable. Reason: timeout.';
    } else {
