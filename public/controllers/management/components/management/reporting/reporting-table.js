/*
 * Psafe app - React component for reporting table.
...
class PaReportingTable extends Component {
  _isMounted = false;
  constructor(props) {
    super(props);
    this.state = {
  ...
  async componentDidUpdate() {
    try {
      ...
      const options = {
        context: `${PaReportingTable.name}.componentDidUpdate`,
  ...
  render() {
    ...
    const deleteReport = (itemList) => {
      try {
        ...
        const options = {
          context: `${PaReportingTable.name}.deleteReport`,
          level: UI_LOGGER_LEVELS.ERROR,
          severity: UI_ERROR_SEVERITIES.BUSINESS,
          error: {
            error: error,
...
export default connect(mapStateToProps, mapDispatchToProps)(PaReportingTable);
