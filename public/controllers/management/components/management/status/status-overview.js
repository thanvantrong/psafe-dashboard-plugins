/*
 * Psafe app - React component for building the status view
 *
...
// Psafe components
import PaStatusActionButtons from './actions-buttons-main';
import PaStatusDaemons from './status-daemons';
import PaStatusStats from './status-stats';
import PaStatusNodeInfo from './status-node-info';
import PaStatusAgentInfo from './status-agent-info';
...
export class PaStatusOverview extends Component {
  _isMounted = false;
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    try {
      ...
      const options = {
        context: `${PaStatusOverview.name}.fetchData`,
      ...
      <EuiPage style={{ background: 'transparent' }}>
        <EuiPanel>
          <EuiFlexGroup>
            ...
            <PaStatusActionButtons />
          </EuiFlexGroup>
          <EuiFlexGroup>
            <EuiFlexItem>
              {isLoading && <EuiProgress size="xs" color="primary" />}
              {!isLoading && listDaemons && <PaStatusDaemons />}
              <EuiSpacer size="l" />
              {!isLoading && stats && <PaStatusStats />}
              <EuiSpacer size="l" />
              {!isLoading && (
