/*
 * Psafe app - React component for render button to refresh cluster nodes info.
...
class PaRefreshClusterInfoButton extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false
  ...
  async checkIfClusterOrManager() {
    try {
      ...
      this.props.updateClusterNodeSelected(false);
      const options = {
        context: `${PaRefreshClusterInfoButton.name}.checkIfClusterOrManager`,
