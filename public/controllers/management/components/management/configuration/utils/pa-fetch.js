/*
 * Psafe app - Fetch API function and utils.
  ...
  try {
    ...
    for (const section of sections) {
      ...
      try {
        ...
        const partialResult = await PaRequest.apiReq('GET', url, {});
...
export const extractMessage = error => {
  if ((error || {}).status === -1) {
    ...
    return isFromAPI ?
      'Psafe API is not reachable. Reason: timeout.' :
 ...
 * @param {*} location
 * @param updatePsafeNotReadyYet
 * @param {boolean} isCluster
 */
export const handleError = async (error, location, updatePsafeNotReadyYet, isCluster) => {
  const message = extractMessage(error);
  const messageIsString = typeof message === 'string';
  try {
    if (messageIsString && message.includes('ERROR3099')) {
      updatePsafeNotReadyYet('Psafe not ready yet.');
      await makePing(updatePsafeNotReadyYet, isCluster);
      return;
    }
...
export const checkDaemons = async (isCluster) => {
  try {
    const response = await PaRequest.apiReq('GET', '/manager/status', {}, { checkCurrentApiIsUp: false });
    ...
    if (isCluster) {
      ...
      clusterd = clusterStatus.enabled === 'yes' && clusterStatus.running === 'yes'
        ? daemons['psafe-clusterd'] === 'running'
        : false;
    }

    const isValid = execd && modulesd && psafedb && (isCluster ? clusterd : true);

    if (isValid) {
      return { isValid };
    } else {
      console.warn('Psafe not ready yet');
    }
  } catch (error) {
    throw error;
  }
...
/**
 * Make ping to Psafe API
 * @param updatePsafeNotReadyYet
 * @param {boolean} isCluster
 * @param {number} [tries=10] Tries
 * @return {Promise}
 */
export const makePing = async (updatePsafeNotReadyYet, isCluster, tries = 30) => {
  try {
    let isValid = false;
    while (tries--) {
    ...
    return Promise.resolve('Psafe is ready');
  } catch (error) {
    throw new Error('Psafe could not be recovered.');
  }
};

/**
 * Get Cluster status from Psafe API
 * @returns {Promise}
 */
export const clusterReq = async () => {
  try {
    return PaRequest.apiReq('GET', '/cluster/status', {});
  } catch (error) {
...
export const fetchFile = async selectedNode => {
  try {
    ...
    const data = await PaRequest.apiReq(
      'GET',
      isCluster ?
 ...
 * @param updatePsafeNotReadyYet
 */
export const restartNodeSelected = async (selectedNode, updatePsafeNotReadyYet) => {
  try {
    const clusterStatus = (((await clusterReq()) || {}).data || {}).data || {};
    const isCluster = clusterStatus.enabled === 'yes' && clusterStatus.running === 'yes';
    // Dispatch a Redux action
    updatePsafeNotReadyYet(`Restarting ${isCluster ? selectedNode : 'Manager'}, please wait.`); //FIXME: if it enables/disables cluster, this will show Manager instead node name
    isCluster ? await restartNode(selectedNode) : await restartManager();
    return await makePing(updatePsafeNotReadyYet, isCluster);
  } catch (error) {
    throw error;
  }
};
...
export const restartCluster = async () => {
  try {
    ...
    await PaRequest.apiReq('PUT', `/cluster/restart`, {
      delay: 15000
    });
...
/**
 * Save text to optit.conf cluster file
...
/**
 * Save text to optit.conf manager file
 * @param {string} text Text to save
 */
...
export const validateAfterSent = async (node = false) => {
  try {
    const clusterStatus = await PaRequest.apiReq('GET', `/cluster/status`, {});
    ...
    if (node && isCluster) {
      validation = await PaRequest.apiReq(
        'GET',
        `/cluster/configuration/validation`, {}
      );
    } else {
      validation = isCluster
        ? await PaRequest.apiReq('GET', `/cluster/configuration/validation`, {})
        : await PaRequest.apiReq(
            'GET',
            `/manager/configuration/validation`,
            {}
...
export const agentIsSynchronized = async agent => {
  const isSync = await PaRequest.apiReq(
    'GET',
...
export const clusterNodes = async () => {
  try {
    const result = await PaRequest.apiReq('GET', `/cluster/nodes`, {});
    return result;
  } catch (error) {
    throw error;
  }
 ...
 */
export const restartClusterOrManager = async (updatePsafeNotReadyYet) => {
  try {
    const clusterStatus = (((await clusterReq()) || {}).data || {}).data || {};
    const isCluster = clusterStatus.enabled === 'yes' && clusterStatus.running === 'yes';
    getToasts().add({
    ...
    // Dispatch a Redux action
    updatePsafeNotReadyYet(`Restarting ${isCluster ? 'Cluster' : 'Manager'}, please wait.`);
    await makePing(updatePsafeNotReadyYet, isCluster);
