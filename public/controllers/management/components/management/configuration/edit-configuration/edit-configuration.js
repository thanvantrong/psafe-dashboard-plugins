/*
 * Psafe app - React component for show Edit configuration.
...
import PaCodeEditor from '../util-components/code-editor';
import PaPsafeAPINotReachable from '../util-components/pa-api-not-reachable';
...
import { PaButtonPermissions } from '../../../../../../components/common/permissions/button';
...
class PaEditConfiguration extends Component {
  constructor(props) {
    super(props);
    this.state = {
      xml: '',
  ...
  async editorSave() {
    try {
      ...
      const options = {
        context: `${PaEditConfiguration.name}.editorSave`,
  ...
  refresh() {
    try {
      ...
      const options = {
        context: `${PaEditConfiguration.name}.refresh`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
  ...
  async confirmRestart() {
    try {
      ...
      await restartNodeSelected(this.props.clusterNodeSelected, this.props.updatePsafeNotReadyYet);
      this.props.updatePsafeNotReadyYet('');
      ...
      this.setState({ restart: false, saving: false, restarting: false });
      const options = {
        context: `${PaEditConfiguration.name}.confirmRestart`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
...
PaEditConfiguration.propTypes = {
  psafeNotReadyYet: PropTypes.oneOfType([PropTypes.bool, PropTypes.string]),
  updatePsafeNotReadyYet: PropTypes.func,
};

export default connect(mapStateToProps, mapDispatchToProps)(PaEditConfiguration);
...
const PaEditorConfiguration = compose(
  ...
  class PaEditorConfiguration extends Component {
    constructor(props) {
      super(props);
    }
    componentDidMount() {
      ...
      return (
        <Fragment>
          {!this.props.errorXMLFetched ? (
            <Fragment>
              <EuiText>
                Edit <span style={{ fontWeight: 'bold' }}>optit.conf</span> of{' '}
              ...
              {typeof editorValue === 'string' && (
                <PaCodeEditor
                  ...
                  minusHeight={psafeNotReadyYet || infoChangesAfterRestart ? 320 : 270}
                />
              )}
            </Fragment>
          ) : (
            <PaPsafeAPINotReachable error={this.props.errorXMLFetched} />
          )}
        </Fragment>
      );
    }
