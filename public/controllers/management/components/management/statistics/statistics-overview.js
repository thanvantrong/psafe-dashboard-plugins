/*
 * Psafe app - React component for building the reporting view
 *
...
import { clusterNodes } from "../configuration/utils/pa-fetch";
import { PaStatisticsRemoted } from "./statistics-dashboard-remoted";
import { PaStatisticsAnalysisd } from "./statistics-dashboard-analysisd";
...
const paConfig = new PsafeConfig();

export class PaStatisticsOverview extends Component {
  _isMounted = false;
  constructor(props) {
    super(props);
    this.state = {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      const options = {
        context: `${PaStatisticsOverview.name}.componentDidMount`,
            ...
            <>
              {this.state.selectedTabId === "remoted" && !this.state.loadingNode && (
                <div>
                  ...
                  />
                  <EuiSpacer size={"m"} />
                  <PaStatisticsRemoted clusterNodeSelected={this.state.clusterNodeSelected} refreshVisualizations={this.state.refreshVisualizations}/>
                </div>
              )}
              {this.state.selectedTabId === "analysisd" && !this.state.loadingNode && (
                <div>
                  ...
                  />
                  <EuiSpacer size={"m"} />
                  <PaStatisticsAnalysisd clusterNodeSelected={this.state.clusterNodeSelected} refreshVisualizations={this.state.refreshVisualizations}/>
                </div>
              )}
            </>
          )}
...
export default compose(
  ...
  withGuard(props => {
    return !((paConfig.getConfig() || {})['cron.statistics.status']); // if 'cron.statistics.status' is false, then it renders PromptStatisticsDisabled component
  ...
  props => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try{
          setLoading(true);
          const data = await PaRequest.genericReq('GET', '/elastic/statistics');
          setExistStatisticsIndices(data.data);
        }catch(error){
          setLoading(false);
          const options = {
            context: `${PaStatisticsOverview.name}.fetchData`,
            level: UI_LOGGER_LEVELS.ERROR,
            severity: UI_ERROR_SEVERITIES.BUSINESS,
            error: {
              error: error,
    ...
    return existStatisticsIndices ? <PaStatisticsOverview {...props}/> : <PromptStatisticsNoIndices {...props}/>
  }
);
