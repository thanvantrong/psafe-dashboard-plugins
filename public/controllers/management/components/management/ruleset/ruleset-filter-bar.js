...
class PaRulesetFilterBar extends Component {
  constructor(props) {
    super(props);
  ...
  async buildSelectedOptions(filters) {
    try {
      ...
      //const result = await this.paReq.apiReq('GET', this.paths[section], {})
      if (Object.keys(filters).length) await this.fetchItems(filters);
    } catch (error) {
      const options = {
        context: `${PaRulesetFilterBar.name}.buildSelectedOptions`,
  ...
  async cleanCurrentOption(selectedOptions) {
    try {
      ...
      const options = {
        context: `${PaRulesetFilterBar.name}.cleanCurrentOption`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.UI,
        error: {
          error: error,
