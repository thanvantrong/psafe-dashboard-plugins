...
class PaListEditor extends Component {
  constructor(props) {
    super(props);
    this.state = {
      items: [],
  ...
  async saveList(name, path, addingNew = false) {
    try {
      ...
      const options = {
        context: `${PaListEditor.name}.saveList`,
  ...
  render() {
    ...
    const exportToCsv = async () => {
      try {
        ...
        const options = {
          context: `${PaListEditor.name}.exportToCsv`,
          level: UI_LOGGER_LEVELS.ERROR,
          severity: UI_ERROR_SEVERITIES.BUSINESS,
          error: {
            error: error,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updatePsafeNotReadyYet: (psafeNotReadyYet) =>
      dispatch(updatePsafeNotReadyYet(psafeNotReadyYet)),
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(PaListEditor);
