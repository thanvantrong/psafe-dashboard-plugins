...
import { filtersToObject } from '../../../../../components/pa-search-bar';
import { withUserPermissions } from '../../../../../components/common/hocs/withUserPermissions';
import { PaUserPermissions } from '../../../../../react-services/pa-user-permissions';
import { compose } from 'redux';
import { UI_ERROR_SEVERITIES } from '../../../../../react-services/error-orchestrator/types';
import { UI_LOGGER_LEVELS } from '../../../../../../common/constants';
import { getErrorOrchestrator } from '../../../../../react-services/common-services';
class PaRulesetTable extends Component {
  _isMounted = false;
  constructor(props) {
    super(props);
    this.paReq = (...args) => PaRequest.apiReq(...args);
    this.state = {
      items: [],
      pageSize: 15,
      pageIndex: 0,
    ...
    if (this.props.state.section === RulesetResources.RULES) {
      ...
      if (match && match[0]) {
        ...
        try {
          ...
          const options = {
            context: `${PaRulesetTable.name}.componentDidMount`,
  ...
  async componentDidUpdate(prevProps) {
    ...
    try {
      ...
      const options = {
        context: `${PaRulesetTable.name}.componentDidUpdate`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
  ...
  async getItems() {
    ...
    const rawItems = await this.paReq(
      'GET',
  ...
  async setDefaultItems() {
    try {
      const requestDefaultItems = await this.paReq('GET', '/manager/configuration', {
      ...
      const getRowProps = (item) => {
        ...
        const updateInfo = async () => {
          ...
          try {
            ...
            const options = {
              context: `${PaRulesetTable.name}.updateInfo`,
              level: UI_LOGGER_LEVELS.ERROR,
              severity: UI_ERROR_SEVERITIES.BUSINESS,
              error: {
                error: error,
        ...
        return {
          ...
          onClick: !PaUserPermissions.checkMissingUserPermissions(
  ...
  async removeItems(items) {
    try {
      ...
      const options = {
        context: `${PaRulesetTable.name}.removeItems`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
