...
class PaRulesetActionButtons extends Component {
  constructor(props) {
    super(props);
  ...
  async generateCsv() {
    try {
      ...
      const mapFilters = filters.map((filter) => ({
        name: filter.field,
        value: filter.value,
      })); // adapt to shape used in /api/csv file: server/controllers/psafe-api.js
      ...
      const options = {
        context: `${PaRulesetActionButtons.name}.generateCsv`,
  ...
  async toggleFiles() {
    try {
      ...
      const options = {
        context: `${PaRulesetActionButtons.name}.toggleFiles`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        store: false,
        error: {
...
export default connect(mapStateToProps, mapDispatchToProps)(PaRulesetActionButtons);
