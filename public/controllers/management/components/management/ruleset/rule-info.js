...
import PaTextWithTooltipTruncated from '../../../../../components/common/pa-text-with-tooltip-if-truncated';
import { UI_ERROR_SEVERITIES } from '../../../../../react-services/error-orchestrator/types';
import { UI_LOGGER_LEVELS } from '../../../../../../common/constants';
import { getErrorOrchestrator } from '../../../../../react-services/common-services';

class PaRuleInfo extends Component {
  constructor(props) {
    super(props);
    ...
    const handleFileClick = async (event, {filename, relative_dirname}) => {
      ...
      try {
        ...
        const options = {
          context: `${PaRuleInfo.name}.handleFileClick`,
  ...
  buildComplianceBadges(item) {
    ...
    try {
      ...
      const options = {
        context: `${PaRuleInfo.name}.buildComplianceBadges`,
        level: UI_LOGGER_LEVELS.ERROR,
  ...
  getFormattedDetails(value) {
    if (Array.isArray(value) && value[0].type) {
      ...
      return <PaTextWithTooltipTruncated position="top">{value}</PaTextWithTooltipTruncated>;
    }
  }
  ...
  async getTacticsNames(tactics) {
    try {
      let tacticsObj = [];
      const data = await PaRequest.apiReq('GET', '/mitre/tactics', {
        params: {
    ...
    try {
      ...
      const mitreTactics = await Promise.all(
        compliance.map(async (i) => {
          const data = await PaRequest.apiReq('GET', '/mitre/techniques', {
            params: {
              q: `external_id=${i}`,
            },
          });
      ...
      const options = {
        context: `${PaRuleInfo.name}.addMitreInformation`,
        level: UI_LOGGER_LEVELS.ERROR,
        severity: UI_ERROR_SEVERITIES.BUSINESS,
        error: {
          error: error,
...
export default connect(mapStateToProps, mapDispatchToProps)(PaRuleInfo);
