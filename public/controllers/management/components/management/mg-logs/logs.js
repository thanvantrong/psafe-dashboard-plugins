/*
 * Psafe app - Component what renders Management/Logs
...
import exportCsv from '../../../../../react-services/pa-csv';
...
import { PaFieldSearch } from '../../../../../components/pa-field-search-bar/pa-field-search-bar';
...
export default compose(
  ...
  class PaLogs extends Component {
    constructor(props) {
      super(props);
      this.state = {
        isCluster: false,
    ...
    async componentDidMount() {
      try {
        ...
        const options = {
          context: `${PaLogs.name}.componentDidMount`,
    ...
    async initDaemonsList(logsPath) {
      try {
        ...
        const responseLogsSummary = await PaRequest.apiReq('GET', path, {});
    ...
    async getFullLogs(customOffset = 0) {
      ...
      try {
        const tmpResult = await PaRequest.apiReq('GET', logsPath, {
    ...
    async setFullLogs() {
      try {
        ...
        const options = {
          context: `${PaLogs.name}.setFullLogs`,
          level: UI_LOGGER_LEVELS.ERROR,
    ...
    /**
     * Returns an object with the path to request Psafe logs, the list of nodes and the current selected node.
     */
    async getLogsPath() {
      try {
        ...
        if (clusterEnabled) {
          let nodeList = '';
          let selectedNode = '';
          const nodeListTmp = await PaRequest.apiReq('GET', '/cluster/nodes', {});
    ...
    getNodeList() {
      try {
        ...
        const options = {
          context: `${PaLogs.name}.getNodeList`,
          level: UI_LOGGER_LEVELS.ERROR,
          severity: UI_ERROR_SEVERITIES.BUSINESS,
          store: true,
          error: {
    ...
    exportFormatted = async () => {
      try {
        ...
        await exportCsv(
          ...
          `psafe-${this.state.selectedNode ? `${this.state.selectedNode}-` : ''}optit-log`
        );
      } catch (error) {
        const options = {
          context: `${PaLogs.name}.exportFormatted`,
          level: UI_LOGGER_LEVELS.ERROR,
          severity: UI_ERROR_SEVERITIES.BUSINESS,
          store: true,
          error: {
          ...
          <EuiFlexGroup>
            <EuiFlexItem>
              <EuiTextColor color="subdued">
                <p>List and filter Psafe logs.</p>
              </EuiTextColor>
            </EuiFlexItem>
          </EuiFlexGroup>
          <EuiFlexGroup>
