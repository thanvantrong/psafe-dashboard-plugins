/*
 * Psafe app - Cluster monitoring controller
...
export function ClusterController(
  ...
  $scope.search = term => {
    $scope.$broadcast('psafeSearch', { term });
  };
  ...
  //listeners
  $scope.$on('psafeShowClusterNode', async (event, parameters) => {
    try {
      tabVisualizations.assign({
        monitoring: 1
      });
      $scope.currentNode = parameters.node;
      const data = await PaRequest.apiReq('GET', '/cluster/healthcheck', {
  ...
  const clusterStatus = async () => {
    try {
      const status = await PaRequest.apiReq('GET', '/cluster/status', {});
      $scope.authorized = true;
      return status;
    } catch (error) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await Promise.all([
        PaRequest.apiReq('GET', '/cluster/nodes', {}),
        PaRequest.apiReq('GET', '/cluster/local/config', {}),
        PaRequest.apiReq('GET', '/', {}),
        PaRequest.apiReq('GET', '/agents', { limit: 1 }),
        PaRequest.apiReq('GET', '/cluster/healthcheck', {})
      ]);
