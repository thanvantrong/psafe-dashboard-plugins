/*
 * Psafe app - React component for Logtest.
...
import { PaButtonPermissionsModalConfirm } from '../../../components/common/buttons';
import {
  UI_ERROR_SEVERITIES,
  UIErrorLog,
  UIErrorSeverity,
...
import { PaFlyout } from '../../../components/common/flyouts';
...
export const Logtest = compose(
  ...
  // Format the result of the Psafe API response to an output similar one to the `psafe-logtest` utility
  const formatResult = (result, alert, messages) => {
    // How to the `psafe-logtest` utility logs the output:
    // https://github.com/psafe/psafe/blob/master/framework/scripts/psafe-logtest.py#L359-L397
    ...
    // Rule debugging
    // The output has data if the utility is ran in verbose mode: `psafe-logtest -v`.
    // At this moment, the Psafe API doesn't let run in verbose mode.
  ...
  const runAllTests = async () => {
    ...
    try {
      for (let event of events) {
        const response = await PaRequest.apiReq('PUT', '/logtest', {
          log_format: 'syslog',
          location: 'logtest',
          event,
          ...(token ? { token } : {}),
  ...
  const deleteToken = async () => {
    try {
      await PaRequest.apiReq('DELETE', `/logtest/sessions/${sessionToken}`, {});
    ...
    return (
      <Fragment>
        ...
        <EuiFlexGroup justifyContent="spaceBetween">
          ...
          <EuiFlexItem grow={false}>
            <PaButtonPermissionsModalConfirm
  ...
  return (
    <Fragment>
      {(!props.onFlyout && (
        ...
        <PaFlyout flyoutProps={{ className: 'paApp' }} onClose={() => props.openCloseFlyout()}>
