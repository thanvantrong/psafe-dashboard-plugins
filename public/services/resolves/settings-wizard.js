/*
 * Psafe app - Module to execute some checks on most app routes
...
export function settingsWizard(
  ...
  try {
    ...
    const checkResponse = (data) => {
      let fromPsafeHosts = false;
      if (parseInt(data.data.error) === 2) {
        !disableErrors &&
          ErrorHandler.handle('Please set up Psafe API credentials.', '', { warning: true });
      } else if (
        ...
        fromPsafeHosts = true;
        paMisc.setBlankScr(ErrorHandler.handle(data));
        AppState.removeCurrentAPI();
      }

      if (!fromPsafeHosts) {
        paMisc.setWizard(true);
        ...
        if (
          ...
          !disableErrors &&
            ErrorHandler.handle(
              'Wrong Psafe API credentials, please add a new API and/or modify the existing one'
            );
          if (!$location.path().includes('/settings')) {
            $location.search('_a', null);
            $location.search('tab', 'api');
    ...
    const setUpCredentials = (msg, redirect = false) => {
      const comeFromWizard = paMisc.getWizard();
          ...
          try {
            ...
            if (errors >= apis.length) {
              ...
              AppState.setNavigation({
                ...
                discoverSections: ['/overview/', '/agents', '/psafe-dev'],
              });
        ...
        genericReq
          ...
          .then(async (data) => {
            if (data.data.length > 0) {
              ...
              setUpCredentials('Psafe App: Default API has been updated.', defaultApi);
              $location.path('health-check');
            } else {
              setUpCredentials('Psafe App: Please set up Psafe API credentials.');
            }
            deferred.resolve();
          })
          .catch((error) => {
          ...
          .then(async (data) => {
            if (data.data.length > 0 && data.data.find((api) => api.id == apiId)) {
              ...
              if (data.data.length > 0) {
                ...
                setUpCredentials('Psafe App: Please set up Psafe API credentials.', false);
              }
            }
          })
          .catch((error) => {
