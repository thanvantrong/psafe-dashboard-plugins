/*
 * Psafe app - Module to fetch index patterns
...
import { getPaConfig } from './get-config';
...
export function getIp(
  ...
  const checkPsafePatterns = async (indexPatterns) => {
    const psafeConfig = new PsafeConfig();
    const configuration = await getPaConfig($q, GenericRequest, psafeConfig);
    const psafePatterns = [
      `${configuration['psafe.monitoring.pattern']}`,
      `${configuration['cron.prefix']}-${configuration['cron.statistics.index.name']}-*`
    ];
    return psafePatterns.every(pattern => {
  ...
  const buildSavedObjectsClient = async () => {
    try {
      ...
      if (
        ...
        !(await checkPsafePatterns(savedObjects))
      ) {
      ...
      const onlyPsafeAlerts = savedObjects.filter(
        element => element.id === currentPattern
      );

      if (!onlyPsafeAlerts || !onlyPsafeAlerts.length) {
      ...
      const options = {
        context: `${getIp.name}.checkPsafePatterns`,
