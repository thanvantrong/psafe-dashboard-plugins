...
export class ErrorHandler {
  ...
  handle(error, location, isWarning, silent) {
    ...
    if (messageIsString && message.includes('ERROR3099')) {
      const updateNotReadyYet = updatePsafeNotReadyYet('Psafe not ready yet.');
      store.dispatch(updateNotReadyYet);

      this.$rootScope.psafeNotReadyYet = 'Psafe not ready yet.';
    ...
    if (this.paMisc.getBlankScr()) silent = true;
