...
export class ReportingService {
  ...
  removeAgentStatusVis(idArray) {
    const monitoringEnabled = this.psafeConfig.getConfig()['psafe.monitoring.enabled'];
    if (!monitoringEnabled) {
      const visArray = idArray.filter((vis) => {
        return vis !== 'Psafe-App-Overview-General-Agents-status';
      });
      return visArray;
    }
    return idArray;
  ...
  async startVis2Png(tab, isAgents = false, syscollectorFilters = null) {
    try {
      ...
      const name = `psafe-${isAgents ? 'agents' : 'overview'}-${tab}-${
        (Date.now() / 1000) | 0
      }.pdf`;
      ...
      this.$rootScope.$applyAsync();
      ErrorHandler.info('Success. Go to Psafe > Management > Reporting', 'Reporting');
  ...
  async startConfigReport(obj, type, components) {
    try {
      ...
      const docType = type === 'agentConfig' ? `psafe-agent-${obj.id}` : `psafe-group-${obj.name}`;
