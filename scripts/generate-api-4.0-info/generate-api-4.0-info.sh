...
main(){
  if [ -z ${PSAFE_API_URL+x} ]; then
cat <<EOF
ERROR: PSAFE_API_URL variable is not defined.
If you are using npm script to launch it, use:
  PSAFE_API_URL="PSAFE_API_URL" npm run <script_name>
  Example: PSAFE_API_URL="https://172.16.1.2:55000" npm run <script_name>
EOF
    exit 1
  fi
  echo "Generate Psafe API 4.0 endpoints data and format to use in Psafe app";
  local API_TMP_OUTPUT_PATH="output";
  local API_OUTPUT_PATH="../../common/api-info";

  node generate-api-4.0-info.js $PSAFE_API_URL --full || exit_with_message "ERROR: the script had an error";
  ...
  echo "Success generating Psafe API 4.0 API info!";
}
