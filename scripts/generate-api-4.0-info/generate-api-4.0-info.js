/*
  Description: this script generates a file with the extrated data of Psafe API 4.0, formatted to use in Psafe app.

  Requirements:
    - Psafe manager 4.0 up

  Use: node generate-api-4.0-info.js PSAFE_API_URL [-f filename]
...
// Constants
const PSAFE_API_URL = process.argv[2]; // Psafe API url is a required argument
...
const PSAFE_DOCUMENTATION_API_REFERENCE_URL = `https://documentation.psafe.com/${PLUGIN_VERSION_SHORT}/user-manual/api/reference.html`;
...
// Main method
const main = () => {
  // Check Psafe API url argument is defined
  if(!PSAFE_API_URL){
    exitWithMessage('Psafe API url is not defined.');
  };
  // Check Psafe API url argument is valid
  if(!PSAFE_API_URL.startsWith('http') ){
    exitWithMessage(`Psafe API url is not valid. It should start with "http". Example: https://172.16.1.2:55000`);
  };
  
  // Log the configuration:
  console.log('--------------- Configuration ---------------');
  console.log(`Psafe API url: ${PSAFE_API_URL}`);
...
const generateAPIEndpointsInformation = async () => {
  try{
    ...
    const apiData = await request(`${PSAFE_API_URL}/openapi.json`);
...
const generateAPISecurityActionsInformation = async () => {
  ...
  const username = 'psafe';
  const password = 'psafe';
  try{
    const authenticationResponse = await request(`${PSAFE_API_URL}/security/user/authenticate`, {
      headers: {
    ...
    const securityActionsResponse = await request(`${PSAFE_API_URL}/security/actions`, {
      headers: {
...
// Formatters
// Format the endpoint to use in the Psafe app
...
const generateEndpointDocumentationLink = endpointData => `${PSAFE_DOCUMENTATION_API_REFERENCE_URL}#operation/${endpointData.operationId}`;
