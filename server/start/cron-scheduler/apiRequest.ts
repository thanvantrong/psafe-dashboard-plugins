  ...
  public async getData():Promise<object> {
    try {
      ...
      if (error.response && error.response.status === 401){
        throw {error: 401, message: 'Wrong Psafe API credentials used'};
      }
      if (error && error.data && error.data.detail && error.data.detail === 'ECONNRESET') {
        throw {error: 3005, message: 'Wrong protocol being used to connect to the Psafe API'};
      }
      if (error && error.data && error.data.detail && ['ENOTFOUND','EHOSTUNREACH','EINVAL','EAI_AGAIN','ECONNREFUSED'].includes(error.data.detail)) {
        throw {error: 3005, message: 'Psafe API is not reachable. Please check your url and port.'};
      }
      throw error;
    }
  }
