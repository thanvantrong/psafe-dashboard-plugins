...
const psafeHostsController = new PsafeHostsCtrl();
...
export class SchedulerJob {
  ...
  constructor(jobName: string, context) {
    ...
    this.logger = context.psafe.logger;
    this.apiClient = context.psafe.api.client.asInternalUser;
  ...
  private async getApiObjects() {
    ...
    const hostsResponse: {body: IApi[]} = await psafeHostsController.getHostsEntries(false, false, fakeResponseEndpoint);
    if (!hostsResponse.body.length) throw {error: 10001, message: 'No Psafe host configured in psafe.yml' }
    if(apis && apis.length){
