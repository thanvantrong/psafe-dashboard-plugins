...
import { PSAFE_STATISTICS_DEFAULT_INDICES_SHARDS, PSAFE_STATISTICS_DEFAULT_INDICES_REPLICAS } from '../../../common/constants';
...
export class SaveDocument {
        ...
        if (!exists.body) {
          const response = await this.esClientInternalUser.indices.create({
            ...
            body: {
              settings: {
                index: {
                  number_of_shards: shards ?? PSAFE_STATISTICS_DEFAULT_INDICES_SHARDS,
                  number_of_replicas: replicas ?? PSAFE_STATISTICS_DEFAULT_INDICES_REPLICAS
                }
              }
            }
          });
  ...
  private addIndexPrefix(index): string {
    ...
    const prefix = configFile['cron.prefix'] || 'psafe';
    return `${prefix}-${index}`;
  }
