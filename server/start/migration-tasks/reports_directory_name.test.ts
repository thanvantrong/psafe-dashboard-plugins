...
describe('[migration] Rename the subdirectories of `reports` directory', () => {
  ...
  afterEach(() => {
    mockContext = null;
    execSync(`rm -rf ${PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}`);
  });
    ...
    directories.forEach(({ name, files }) => {
      ...
      if (files) {
        Array.from(Array(files).keys()).forEach(indexFile => {
          fs.closeSync(fs.openSync(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name, `report_${indexFile}.pdf`), 'w'));
        });
      }
    });
    ...
    directories.forEach(({ name, ...rest }) => {
      if (!rest.errorRenaming) {
        if (isMD5(name)) {
          ...
          expect(fs.existsSync(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name))).toBe(true);
        } else {
          ...
          expect(fs.existsSync(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, md5(name)))).toBe(true);
          expect(!fs.existsSync(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name))).toBe(true);
        };
      } else {
