...
import { PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH } from '../../../common/constants';
...
export default function migrateReportsDirectoryName(context) {
  ...
  const createLog = (level: string) => (message) => {
    ...
    context.psafe.logger[level](`migration:reportsDirectoryName: ${message}`);
  };
  ...
  try {
    ...
    if (!fs.existsSync(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH)) {
      logger.debug("Reports directory doesn't exist. The task is not required. Skip.");
      return;
    };

    // Read the directories/files in the reports path
    logger.debug(`Reading reports directory: ${PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH}`);
    fs.readdirSync(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, { withFileTypes: true })
      .forEach((fileDirent) => {
        ...
        if (fileDirent.isDirectory() && !isMD5(fileDirent.name)) {
          ...
          const originDirectoryPath = path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, fileDirent.name);
          const targetDirectoryName = md5(fileDirent.name);
          const targetDirectoryPath = path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, targetDirectoryName);
          try {
