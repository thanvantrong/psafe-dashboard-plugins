/*
 * Psafe app - Module for agent info fetching functions
...
import { 
  ...
  PSAFE_MONITORING_TEMPLATE_NAME,
...
const monitoringErrorLogColors = [bluePsafe, 'monitoring', 'error'];
const psafeHostController = new PsafeHostsCtrl();
...
function initMonitoringConfiguration(context){
  try{
    ...
    MONITORING_ENABLED = appConfig && typeof appConfig['psafe.monitoring.enabled'] !== 'undefined'
      ? appConfig['psafe.monitoring.enabled'] &&
        appConfig['psafe.monitoring.enabled'] !== 'worker'
      : PSAFE_MONITORING_DEFAULT_ENABLED;
    MONITORING_FREQUENCY = getAppConfigurationSetting('psafe.monitoring.frequency', appConfig, PSAFE_MONITORING_DEFAULT_FREQUENCY);
    MONITORING_CRON_FREQ = parseCron(MONITORING_FREQUENCY);
    MONITORING_CREATION = getAppConfigurationSetting('psafe.monitoring.creation', appConfig, PSAFE_MONITORING_DEFAULT_CREATION);

    MONITORING_INDEX_PATTERN = getAppConfigurationSetting('psafe.monitoring.pattern', appConfig, PSAFE_MONITORING_PATTERN);
    ...
    log(
      ...
      `psafe.monitoring.enabled: ${MONITORING_ENABLED}`,
      'debug'
    );

    log(
      'monitoring:initMonitoringConfiguration',
      `psafe.monitoring.frequency: ${MONITORING_FREQUENCY} (${MONITORING_CRON_FREQ})`,
      'debug'
    );

    log(
      'monitoring:initMonitoringConfiguration',
      `psafe.monitoring.pattern: ${MONITORING_INDEX_PATTERN} (index prefix: ${MONITORING_INDEX_PREFIX})`,
      'debug'
    );
  }catch(error){
    ...
    context.psafe.logger.error(errorMessage)
  }
};
  ...
  try {
    ...
    try {
      ...
      const currentTemplate = await context.core.elasticsearch.client.asInternalUser.indices.getTemplate({
        name: PSAFE_MONITORING_TEMPLATE_NAME
      });
      // Copy already created index patterns
      monitoringTemplate.index_patterns = currentTemplate.body[PSAFE_MONITORING_TEMPLATE_NAME].index_patterns;
    }catch (error) {
      // Init with the default index pattern
      monitoringTemplate.index_patterns = [PSAFE_MONITORING_PATTERN];
    }
    ...
    context.psafe.logger.error(monitoringErrorLogColors, errorMessage);
    throw error;
  }
}
...
async function insertMonitoringDataElasticsearch(context, data) {
  const monitoringIndexName = MONITORING_INDEX_PREFIX + indexDate(MONITORING_CREATION);
    ...
    try {
      ...
      context.psafe.logger.error(error.message);
    }
}
 ...
 * @param {String} indexName The name for the index (e.g. daily: psafe-monitoring-YYYY.MM.DD)
 * @param {*} data
 */
...
/**
 * Create the psafe-monitoring index
...
async function createIndex(context, indexName: string) {
  try {
    ...
    const IndexConfiguration = {
      settings: {
        index: {
          number_of_shards: getAppConfigurationSetting('psafe.monitoring.shards', appConfig, PSAFE_MONITORING_DEFAULT_INDICES_SHARDS),
          number_of_replicas: getAppConfigurationSetting('psafe.monitoring.replicas', appConfig, PSAFE_MONITORING_DEFAULT_INDICES_REPLICAS)
        }
      }
    };
...
async function getHostsConfiguration() {
  try {
    const hosts = await psafeHostController.getHostsEntries(false, false, fakeResponseEndpoint);
    ...
    log(
      ...
      'There are no Psafe API entries yet',
      'debug'
    );
...
async function cronTask(context) {
  try {
    const templateMonitoring = await context.core.elasticsearch.client.asInternalUser.indices.getTemplate({name: PSAFE_MONITORING_TEMPLATE_NAME});
    ...
    // try {
    //   if (
    //     this.paWrapper.buildingKibanaIndex ||
    //     ((error || {}).status === 404 &&
...
async function getApiInfo(context, apiHost){
  try{
    ...
    const responseIsCluster = await context.psafe.api.client.asInternalUser.request('GET', '/cluster/status', {}, { apiHostID: apiHost.id });
    const isCluster = (((responseIsCluster || {}).data || {}).data || {}).enabled === 'yes';
    if(isCluster){
      const responseClusterInfo = await context.psafe.api.client.asInternalUser.request('GET', `/cluster/local/info`, {},  { apiHostID: apiHost.id });
...
async function fetchAllAgentsFromApiHost(context, apiHost){
  ...
  try{
    ...
    const responseAgentsCount = await context.psafe.api.client.asInternalUser.request(
      'GET',
      '/agents',
      {
        params: {
    ...
    while (agents.length < agentsCount && payload.offset < agentsCount) {
      try{
        ...
        TODO: Improve the performance of request with:
          - Reduce the number of requests to the Psafe API
