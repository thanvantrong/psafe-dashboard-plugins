...
import { PSAFE_DATA_ABSOLUTE_PATH, PSAFE_DATA_CONFIG_DIRECTORY_PATH, PSAFE_DATA_CONFIG_REGISTRY_PATH } from '../../../common/constants';
...
jest.mock('../../lib/get-configuration', () => ({
    getConfiguration: () => ({pattern: 'psafe-alerts-*'})
}));

beforeAll(() => {
  // Create <PLUGIN_PLATFORM_PATH>/data/psafe directory.
  createDataDirectoryIfNotExists();
  // Create <PLUGIN_PLATFORM_PATH>/data/psafe/config directory.
  createDirectoryIfNotExists(PSAFE_DATA_CONFIG_DIRECTORY_PATH);
});
...
describe("[initialize] `psafe-registry.json` not created", () => {
    let mockContext = mockContextCreator('debug');
    afterEach(() => {
        // Remove <PLUGIN_PLATFORM_PATH>/data/psafe/config/psafe-registry file.
        execSync(`rm ${PSAFE_DATA_ABSOLUTE_PATH}/config/psafe-registry.json || echo ""`);
    });

  it("Create registry file with plugin data and empty hosts", async () => {
    // Migrate the directories
    await jobInitializeRun(mockContext);
    const contentRegistry = JSON.parse(fs.readFileSync(PSAFE_DATA_CONFIG_REGISTRY_PATH, 'utf8'));
    
    expect(contentRegistry.name).toMatch('Psafe App');
...
describe("[initialize] `psafe-registry.json` created", () => {
    let testID = 0;
    ...
    beforeEach(() => {
        // Remove <PLUGIN_PLATFORM_PATH>/data/psafe/config/psafe-registry.json.
        execSync(`rm ${PSAFE_DATA_ABSOLUTE_PATH}/config/psafe-registry.json || echo ""`);
        // Create the psafe-registry.json file.
        fs.writeFileSync(PSAFE_DATA_CONFIG_REGISTRY_PATH, JSON.stringify(contentRegistryFile[testID].before), 'utf8');
        testID++;
    });
    ...
    `(`$titleTest:
      ...
      await jobInitializeRun(mockContext);
      const contentRegistryFile = JSON.parse(fs.readFileSync(PSAFE_DATA_CONFIG_REGISTRY_PATH, 'utf8'));
      
      expect(contentRegistryFile.name).toMatch('Psafe App');
