// To launch this file
// yarn test:jest --testEnvironment node --verbose server/routes/psafe-api
...
describe.skip('Psafe API', () => {
  describe('Psafe API - /api/login', () => {
  ...
  describe('Psafe API - /api/check-api', () => {
    ...
    test('[500] Check unknown api', () => {
      ...
      return axios(options).catch(error=> {
        ...
        expect(error.response.data.message.includes('Selected API is no longer available in psafe.yml')).toBe(true);
      });
    });
  });

  describe('Psafe API - /api/check-stored-api', () => {
    test('[200] Check default api returns manager, node, cluster, status and allow_run_as params', () => {
  ...
  describe('Psafe API - /api/request', () => {
    let userToken = null;
    beforeAll(() => {
    ...
    test('[200] Get agents with a not working user token', () => {
      ...
      {
        cookie: `pa-token=null; pa-api=default;`
      }
      );
      return axios(options).catch(error => {
  ...
  describe('Psafe API - /api/extensions', () => {
  ...
  describe('Psafe API - /api/syscollector', () => {
