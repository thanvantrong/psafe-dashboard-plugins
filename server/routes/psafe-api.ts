...
import { PsafeApiCtrl } from '../controllers';
import { schema } from '@kbn/config-schema';

export function PsafeApiRoutes(router: IRouter) {
  const ctrl = new PsafeApiCtrl();
  ...
  // Check if credentials on POST connect to Psafe API. Not storing them!
  // Returns if the psafe-api configuration received in the POST body will work
  router.post({
    path: '/api/check-api',
    validate: {
  ...
  // Returns data from the Psafe API on CSV readable format
  router.post({
    path: '/api/csv',
    validate: {
      body: schema.object({
  ...
  // Return logged in user has psafe disabled by role
  router.get({
    path: '/api/check-psafe',
    validate: false
  },
    async (context, request, response) => ctrl.isPsafeDisabled(context, request, response)
  );
}
