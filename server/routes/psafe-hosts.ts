/*
 * Psafe app - Module for Psafe-API routes
 ...
 */
import { PsafeHostsCtrl } from '../controllers';
import { IRouter } from 'kibana/server';
import { schema } from '@kbn/config-schema';

export function PsafeHostsRoutes(router: IRouter) {
  const ctrl = new PsafeHostsCtrl();

  // Get Psafe-API entries list (Multimanager) from elasticsearch index
  router.get({
      path: '/hosts/apis',
      validate: false
    },
