/*
 * Psafe app - Module to update the configuration file
...
import { initialPsafeConfig } from './initial-psafe-config';
import { PSAFE_DATA_CONFIG_APP_PATH } from '../../common/constants';
...
export class ManageHosts {
  ...
  constructor() {
    this.busy = false;
    this.file = PSAFE_DATA_CONFIG_APP_PATH;
    this.updateRegistry = new UpdateRegistry();
    this.initialConfig = initialPsafeConfig;
  }
  ...
  /**
   * Returns the hosts in the psafe.yml
   */
  async getHosts() {
    try {
      this.checkBusy();
      ...
      if (!fs.existsSync(PSAFE_DATA_CONFIG_APP_PATH)) {
  ...
  /**
   * This function checks if the hosts: key exists in the psafe.yml for preventing duplicate in case of there's not any host defined
   */
  ...
  /**
   * Returns the IDs of the current hosts in the psafe.yml
   */
  ...
  async getHostById(id) {
    try {
      ...
      if(host && !host.length){
        throw new Error('Selected API is no longer available in psafe.yml');
      }
  ...
  /**
   *  Iterate the array with the API entries in given from the .psafe index in order to create a valid array
  ...
  transformIndexedApis(apiEntries) {
    ...
    try {
      ...
      log(
        ...
        'Transforming index API schedule to psafe.yml',
        'debug'
      );
    } catch (error) {
  ...
  /**
   * Receives an array of hosts and checks if any host is already in the psafe.yml, in this case is removed from the received array and returns the resulting array
   * @param {Array} hosts
   */
  ...
  /**
   * Throws an error is the psafe.yml is busy
   */
  checkBusy() {
    if (this.busy)
  ...
  async addSeveralHosts(hosts) {
    try {
      ...
      return 'All APIs entries were migrated to the psafe.yml';
    } catch (error) {
  ...
  /**
   * Delete a host from the psafe.yml
   * @param {Object} req
   */
