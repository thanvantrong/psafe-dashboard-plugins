/*
 * Psafe app - Service which caches the API user allow run as
...
export const APIUserAllowRunAs = {
  ...
  async canUse(apiId: string): Promise<number | never>{
    ...
    if(ApiUserCanUseStatus === API_USER_STATUS_RUN_AS.USER_NOT_ALLOWED){
      const api = await manageHosts.getHostById(apiId);
      throw new Error(`API with host ID [${apiId}] misconfigured. The Psafe API user [${api.username}] is not allowed to use [run_as]. Allow it in the user configuration or set [run_as] host setting with [false] value.`);
    }
...
export enum API_USER_STATUS_RUN_AS{
  ALL_DISABLED = 0, // Psafe HOST and USER API user configured with run_as=false or undefined
  USER_NOT_ALLOWED = 1, // Psafe HOST API user configured with run_as = TRUE in psafe.yml but it has not run_as in Psafe API
  HOST_DISABLED = 2, // Psafe HOST API user configured with run_as=false in psafe.yml but it has not run_as in Psafe API
  ENABLED = 3 // Psafe API user configured with run_as=true and allow run_as
}
