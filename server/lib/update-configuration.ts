...
import { PSAFE_DATA_CONFIG_APP_PATH, PSAFE_CONFIGURATION_SETTINGS_NEED_RESTART, PSAFE_CONFIGURATION_SETTINGS_NEED_RELOAD, PSAFE_CONFIGURATION_SETTINGS_NEED_HEALTH_CHECK } from '../../common/constants';
...
export class UpdateConfigurationFile {
  ...
  /**
   * Add or replace specific setting from psafe.yml
  ...
  formatValueCachedConfiguration = (value) => typeof value === 'string'
    ...
    : value;
  /**
   * Updates psafe.yml file. If it fails, it throws the error to the next function.
   * @param {Object} input
   */
  updateConfiguration(input) {
    try {
      ...
      return {
        needRestart: PSAFE_CONFIGURATION_SETTINGS_NEED_RESTART.includes(key),
        needReload: PSAFE_CONFIGURATION_SETTINGS_NEED_RELOAD.includes(key),
        needHealtCheck: PSAFE_CONFIGURATION_SETTINGS_NEED_HEALTH_CHECK.includes(key)
      };
    } catch (error) {
