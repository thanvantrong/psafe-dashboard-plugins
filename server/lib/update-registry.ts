...
import { PSAFE_DATA_CONFIG_REGISTRY_PATH } from '../../common/constants';
...
export class UpdateRegistry {
  ...
  constructor() {
    this.busy = false;
    this.file = PSAFE_DATA_CONFIG_REGISTRY_PATH;
  }

  /**
   * Reads the Psafe registry content
   */
  async readContent() {
    try {
      log('update-registry:readContent', 'Reading psafe-registry.json content', 'debug');
  ...
  /**
   * Writes the psafe-registry.json
   * @param {Object} content
   */
  async writeContent(content) {
    try {
      log('update-registry:writeContent', 'Writting psafe-registry.json content', 'debug');
      if (this.busy) {
  ...
  /**
   * Compare the hosts from psafe.yml and the host in the psafe-registry.json file in order to remove the orphan registry register
   * @param {Array} hosts
   */
