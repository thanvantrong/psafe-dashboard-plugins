...
import { PSAFE_DATA_LOGS_DIRECTORY_PATH, MAX_MB_LOG_FILES } from '../../common/constants';
...
export class BaseLogger {
  ...
  psafeLogger: winston.Logger | undefined = undefined;
  psafePlainLogger: winston.Logger | undefined = undefined;
  ...
  constructor(plainLogsFile: string, rawLogsFile: string) {
    this.PLAIN_LOGS_PATH = path.join(PSAFE_DATA_LOGS_DIRECTORY_PATH, plainLogsFile);
    this.RAW_LOGS_PATH = path.join(PSAFE_DATA_LOGS_DIRECTORY_PATH, rawLogsFile);
  ...
  private initLogger = () => {
    ...
    // JSON logger
    this.psafeLogger = winston.createLogger({
      level,
    ...
    this.psafeLogger.exitOnError = false;

    // Plain text logger
    this.psafePlainLogger = winston.createLogger({
      level,
    ...
    // Prevents from exit on error related to the logger.
    this.psafePlainLogger.exitOnError = false;
  };

  /**
   * Checks if psafe/logs exists. If it doesn't exist, it will be created.
   */
  initDirectory = async () => {
    try {
      createDataDirectoryIfNotExists();
      createDataDirectoryIfNotExists('logs');
      if (typeof this.psafeLogger === 'undefined' || typeof this.psafePlainLogger === 'undefined') {
        this.initLogger();
      }
      this.allowed = true;
      return;
  ...
  rotateFiles = (file: string, pathFile: string, log?: string) => {
    if (this.getFilesizeInMegaBytes(pathFile) >= MAX_MB_LOG_FILES) {
      ...
      fs.renameSync(
        pathFile,
        `${PSAFE_DATA_LOGS_DIRECTORY_PATH}/${fileName}-${new Date().getTime()}${fileExtension}`
      );
      if (log) {
  ...
  /**
   * Checks if the psafeapp.log file size is greater than 100MB, if so it rotates the file.
   */
  private checkFiles = () => {
    ...
    return this.initDirectory()
      .then(() => {
        if (this.allowed) {
          ...
          this.psafePlainLogger.log(plainLogData);
