/*
 * Psafe app - Module to transform seconds interval to cron readable format
...
import { PSAFE_MONITORING_DEFAULT_CRON_FREQ } from '../../common/constants';
...
export function parseCron(interval: string) {
  try {
    ...
    log(
      'cron:parse-interval',
      `Using default value ${PSAFE_MONITORING_DEFAULT_CRON_FREQ} due to: ${error.message || error}`
    );
    return PSAFE_MONITORING_DEFAULT_CRON_FREQ;
  }
}
