...
import {
  ...
  PSAFE_DATA_PLUGIN_PLATFORM_BASE_ABSOLUTE_PATH,
...
export class PsafeHostsCtrl {
  ...
  /**
   * This get all hosts entries in the psafe.yml and the related info in the psafe-registry.json
  ...
  async getHostsEntries(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      ...
      if(error && error.message && ['ENOENT: no such file or directory', PSAFE_DATA_PLUGIN_PLATFORM_BASE_ABSOLUTE_PATH].every(text => error.message.includes(text))){
        return response.badRequest({
          body: {
            message: `Error getting the hosts entries: The \'${PSAFE_DATA_PLUGIN_PLATFORM_BASE_ABSOLUTE_PATH}\' directory could not exist in your ${PLUGIN_PLATFORM_NAME} installation.
            If this doesn't exist, create it and give the permissions 'sudo mkdir ${PSAFE_DATA_PLUGIN_PLATFORM_BASE_ABSOLUTE_PATH};sudo chown -R ${PLUGIN_PLATFORM_INSTALLATION_USER}:${PLUGIN_PLATFORM_INSTALLATION_USER_GROUP} ${PSAFE_DATA_PLUGIN_PLATFORM_BASE_ABSOLUTE_PATH}'. After, restart the ${PLUGIN_PLATFORM_NAME} service.`
          }
        })
      }
      log('psafe-hosts:getHostsEntries', error.message || error);
  ...
  async joinHostRegistry(hosts: any, registry: any, removePassword: boolean = true) {
    try {
      if (!Array.isArray(hosts)) {
        throw new Error('Hosts configuration error in psafe.yml');
      }
  ...
  async updateClusterInfo(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-hosts:updateClusterInfo', error.message || error);
      return ErrorResponse(
        `Could not update data in psafe-registry.json due to ${error.message || error}`,
        2012,
        500,
        response
      );
  ...
  async removeOrphanEntries(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-hosts:cleanRegistry', 'Cleaning registry', 'debug');
      await this.updateRegistry.removeOrphanEntries(entries);
      return response.ok({
        body: { statusCode: 200, message: 'ok' }
      });
    } catch (error) {
      log('psafe-hosts:cleanRegistry', error.message || error);
      return ErrorResponse(
        `Could not clean entries in the psafe-registry.json due to ${error.message || error}`,
        2013,
        500,
        response
      );
