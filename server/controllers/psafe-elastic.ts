/*
 * Psafe app - Class for Psafe-Elastic functions
...
import { PSAFE_MONITORING_PATTERN, PSAFE_SAMPLE_ALERT_PREFIX, PSAFE_ROLE_ADMINISTRATOR_ID, PSAFE_SAMPLE_ALERTS_INDEX_SHARDS, PSAFE_SAMPLE_ALERTS_INDEX_REPLICAS } from '../../common/constants';
...
import { PSAFE_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS, PSAFE_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS } from '../../common/constants'

export class PsafeElasticCtrl {
  paSampleAlertsIndexPrefix: string
  manageHosts: ManageHosts
  constructor() {
    this.paSampleAlertsIndexPrefix = this.getSampleAlertPrefix();
  ...
  buildSampleIndexByCategory(category: string): string {
    return `${this.paSampleAlertsIndexPrefix}sample-${category}`;
  }
  ...
  getSampleAlertPrefix(): string {
    ...
    return config['alerts.sample.prefix'] || PSAFE_SAMPLE_ALERT_PREFIX;
  }
  ...
  async getTemplate(context: RequestHandlerContext, request: KibanaRequest<{ pattern: string }>, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-elastic:getTemplate', error.message || error);
  ...
  async checkPattern(context: RequestHandlerContext, request: KibanaRequest<{ pattern: string }>, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-elastic:checkPattern', error.message || error);
      return ErrorResponse(
  ...
  async getFieldTop(context: RequestHandlerContext, request: KibanaRequest<{ mode: string, cluster: string, field: string, pattern: string }, { agentsList: string }>, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-elastic:getFieldTop', error.message || error);
  ...
  async getCurrentPlatform(context: RequestHandlerContext, request: KibanaRequest<{ user: string }>, response: KibanaResponseFactory) {
    try {
      return response.ok({
        body: {
          platform: context.psafe.security.platform
        }
      });
    } catch (error) {
      log('psafe-elastic:getCurrentPlatform', error.message || error);
   ...
   * @param {String} id Index-pattern id to use in the visualizations. Eg: 'psafe-alerts'
   */
  async buildVisualizationsRaw(app_objects, id, namespace = false) {
    try {
      const config = getConfiguration();
      let monitoringPattern =
        (config || {})['psafe.monitoring.pattern'] || PSAFE_MONITORING_PATTERN;
      log(
        'psafe-elastic:buildVisualizationsRaw',
      ...
      for (let element of app_objects) {
        ...
        if (
          ...
          const isMonitoring = defaultStr.includes('psafe-monitoring');
          if (isMonitoring) {
      ...
      log('psafe-elastic:buildVisualizationsRaw', error.message || error);
   ...
   * @param {String} name Cluster name. Eg: 'psafe'
  ...
  buildClusterVisualizationsRaw(
    ...
    try {
      ...
      for (const element of app_objects) {
        ...
        aux_source = aux_source.replace(/psafe-alerts/g, id);
        ...
        if (visState.type && visState.type === 'timelion') {
          let query = '';
          if (title === 'Psafe App Cluster Overview') {
            for (const node of nodes) {
              query += `.es(index=${pattern_name},q="cluster.name: ${name} AND cluster.node: ${node.name}").label("${node.name}"),`;
            }
            query = query.substring(0, query.length - 1);
          } else if (title === 'Psafe App Cluster Overview Manager') {
            query += `.es(index=${pattern_name},q="cluster.name: ${name}").label("${name} cluster")`;
          } else {
            if (title.startsWith('Psafe App Statistics')) {
              const { searchSourceJSON } = bulk_content.visualization.kibanaSavedObjectMeta;
              bulk_content.visualization.kibanaSavedObjectMeta.searchSourceJSON = searchSourceJSON.replace('psafe-statistics-*', pattern_name);
            }
            if (title.startsWith('Psafe App Statistics') && name !== '-' && name !== 'all' && visState.params.expression.includes('q=')) {
              ...
              const _visState = bulk_content.visualization.visStateByNode
                ...
                : visState;
              query += _visState.params.expression.replace(/psafe-statistics-\*/g, pattern_name).replace(expressionRegex, `q="nodeName.keyword:${name} AND apiName.keyword:${master_node}"`)
                .replace("NODE_NAME", name)
            } else if (title.startsWith('Psafe App Statistics')) {
              const expressionRegex = /q='\*'/gi
              query += visState.params.expression.replace(/psafe-statistics-\*/g, pattern_name).replace(expressionRegex, `q="apiName.keyword:${master_node}"`)
            } else {
      ...
      log(
        'psafe-elastic:buildClusterVisualizationsRaw',
        error.message || error
      );
      return Promise.reject(error);
    }
  ...
  async createVis(context: RequestHandlerContext, request: KibanaRequest<{ pattern: string, tab: string }>, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-elastic:createVis', `${tabPrefix}[${tabSufix}] with index pattern ${request.params.pattern}`, 'debug');
      const namespace = context.psafe.plugins.spaces && context.psafe.plugins.spaces.spacesService && context.psafe.plugins.spaces.spacesService.getSpaceId(request);
      ...
      log('psafe-elastic:createVis', error.message || error);
  ...
  async createClusterVis(context: RequestHandlerContext, request: KibanaRequest<{ pattern: string, tab: string }, unknown, any>, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-elastic:createClusterVis', error.message || error);
  ...
  async haveSampleAlerts(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      // Check if psafe sample alerts index exists
      const results = await Promise.all(Object.keys(PSAFE_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS)
  ...
  /**
   * This creates sample alerts in psafe-sample-alerts
  ...
  async haveSampleAlertsOfCategory(context: RequestHandlerContext, request: KibanaRequest<{ category: string }>, response: KibanaResponseFactory) {
    try {
      ...
      log(
        'psafe-elastic:haveSampleAlertsOfCategory',
  ...
  async createSampleAlerts(context: RequestHandlerContext, request: KibanaRequest<{ category: string }>, response: KibanaResponseFactory) {
    ...
    try {
      ...
      const sampleAlerts = PSAFE_SAMPLE_ALERTS_CATEGORIES_TYPE_ALERTS[request.params.category].map((typeAlert) => generateAlerts({ ...typeAlert, ...alertGenerateParams }, request.body.alerts || typeAlert.alerts || PSAFE_SAMPLE_ALERTS_DEFAULT_NUMBER_ALERTS)).flat();
      ...
      if (!existsSampleIndex.body) {
        // Create psafe sample alerts index

        const configuration = {
          settings: {
            index: {
              number_of_shards: PSAFE_SAMPLE_ALERTS_INDEX_SHARDS,
              number_of_replicas: PSAFE_SAMPLE_ALERTS_INDEX_REPLICAS
            }
          }
        };
        ...
        log(
          'psafe-elastic:createSampleAlerts',
  ...
  async deleteSampleAlerts(context: RequestHandlerContext, request: KibanaRequest<{ category: string }>, response: KibanaResponseFactory) {
    // Delete Psafe sample alert index
  ...
  async alerts(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      ...
      log('psafe-elastic:alerts', error.message || error);
  ...
  async existStatisticsIndices(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      const config = getConfiguration();
      const statisticsPattern = `${config['cron.prefix'] || 'psafe'}-${config['cron.statistics.index.name'] || 'statistics'}*`; //TODO: replace by default as constants instead hardcoded ('psafe' and 'statistics')
      ...
      log('psafe-elastic:existsStatisticsIndices', error.message || error);
  ...
  async existMonitoringIndices(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      const config = getConfiguration();
      const monitoringIndexPattern = config['psafe.monitoring.pattern'] || PSAFE_MONITORING_PATTERN;
      const existIndex = await context.core.elasticsearch.client.asCurrentUser.indices.exists({
      ...
      log('psafe-elastic:existsMonitoringIndices', error.message || error);
      return ErrorResponse(error.message || error, 1000, 500, response);
    }
  }
