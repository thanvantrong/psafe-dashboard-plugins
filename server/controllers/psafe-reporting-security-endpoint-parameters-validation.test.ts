...
import { PsafeReportingRoutes } from '../routes/psafe-reporting';
import md5 from 'md5';
import { createDataDirectoryIfNotExists, createDirectoryIfNotExists } from '../lib/filesystem';
import { PSAFE_DATA_ABSOLUTE_PATH, PSAFE_DATA_DOWNLOADS_DIRECTORY_PATH, PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH } from '../../common/constants';
...
beforeAll(async () => {
  // Create <PLUGIN_PLATFORM_PATH>/data/psafe directory.
  createDataDirectoryIfNotExists();
  // Create <PLUGIN_PLATFORM_PATH>/data/psafe/downloads directory.
  createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_DIRECTORY_PATH);
  // Create <PLUGIN_PLATFORM_PATH>/data/psafe/downloads/reports directory.
  createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
  // Create report files
  [
    { name: md5('admin'), files: ['psafe-module-overview-general-1234.pdf'] },
    { name: md5('../../etc'), files: ['psafe-module-overview-general-1234.pdf'] }
  ].forEach(({ name, files }) => {
    createDirectoryIfNotExists(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name));

    if (files) {
      files.forEach(filename => fs.closeSync(fs.openSync(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, name, filename), 'w')));
    };
  });
...
afterAll(async () => {
  // Remove <PLUGIN_PLATFORM_PATH>/data/psafe directory.
  execSync(`rm -rf ${PSAFE_DATA_ABSOLUTE_PATH}`);
...
describe('[endpoint][security] GET /reports/{name} - Parameters validation', () => {
  it.each`
        ...
        ${'Get report by filename'} | ${'admin'}     | ${'psafe-module-overview-general-1234.pdf'}               | ${200}             | ${null}
        ${'Invalid parameters'}     | ${'admin'}     | ${'..%2fpsafe-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Invalid parameters'}     | ${'admin'}     | ${'custom..%2fpsafe-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Route not found'}        | ${'admin'}     | ${'../custom..%2fpsafe-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
        ${'Get report by filename'} | ${'../../etc'} | ${'psafe-module-overview-general-1234.pdf'}               | ${200}             | ${null}
        ${'Invalid parameters'}     | ${'../../etc'} | ${'..%2fpsafe-module-overview-general-1234.pdf'}          | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Invalid parameters'}     | ${'../../etc'} | ${'custom..%2fpsafe-module-overview-general-1234.pdf'}    | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Route not found'}        | ${'../../etc'} | ${'../custom..%2fpsafe-module-overview-general-1234.pdf'} | ${404}             | ${/Not Found/}
  ...
  it.each`
    ...
    const response = await supertest(innerServer.listener)
      ...
      .send({
        ...
        tables: [],
        section: 'overview',
        indexPatternTitle: 'psafe-alerts-*',
        apiId: 'default'
      })
      .expect(responseStatusCode);
    if (responseBodyMessage) {
...
describe('[endpoint][security] DELETE /reports/{name} - Parameters validation', () => {
  it.each`
        testTitle               | username       | filename                                                | responseStatusCode | responseBodyMessage
        ${'Delete report file'} | ${'admin'}     | ${'psafe-module-overview-general-1234.pdf'}             | ${200}             | ${null}
        ${'Invalid parameters'} | ${'admin'}     | ${'..%2fpsafe-module-overview-general-1234.pdf'}        | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Invalid parameters'} | ${'admin'}     | ${'custom..%2fpsafe-module-overview-general-1234.pdf'}  | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Route not found'}    | ${'admin'}     | ${'../psafe-module-overview-general-1234.pdf'}          | ${404}             | ${/Not Found/}
        ${'Delete report file'} | ${'../../etc'} | ${'psafe-module-overview-general-1234.pdf'}             | ${200}             | ${null}   
        ${'Invalid parameters'} | ${'../../etc'} | ${'..%2fpsafe-module-overview-general-1234.pdf'}        | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Invalid parameters'} | ${'../../etc'} | ${'custom..%2fpsafe-module-overview-general-1234.pdf'}  | ${400}             | ${'[request params.name]: must be A-z, 0-9, _, ., and - are allowed. It must end with .pdf.'}
        ${'Route not found'}    | ${'../../etc'} | ${'../psafe-module-overview-general-1234.pdf'}          | ${404}             | ${/Not Found/}
