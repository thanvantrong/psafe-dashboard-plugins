/*
 * Psafe app - Class for Psafe reporting controller
...
import {
  ...
  PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH,
  AUTHORIZED_AGENTS,
  API_NAME_AGENT_STATUS,
} from '../../common/constants';
import { createDirectoryIfNotExists, createDataDirectoryIfNotExists } from '../lib/filesystem';
import moment from 'moment';
import { agentStatusLabelByAgentStatus } from '../../common/services/pa_agent_status';

export class PsafeReportingCtrl {
  constructor() {}

  /**
   * This do format to filters
   * @param {String} filters E.g: cluster.name: psafe AND rule.groups: vulnerability
    ...
    try {
      ...
      if (section && typeof section === 'string') {
        if (!['agentConfig', 'groupConfig'].includes(section)) {
          printer.addContent({
            text: PSAFE_MODULES[tab].title + ' report',
            style: 'h1',
          });
      ...
      if (isAgents && typeof isAgents === 'string') {
        const agentResponse = await context.psafe.api.client.asCurrentUser.request(
          'GET',
          `/agents`,
      ...
      if (PSAFE_MODULES[tab] && PSAFE_MODULES[tab].description) {
        printer.addContentWithNewLine({
          text: PSAFE_MODULES[tab].description,
          style: 'standard',
        });
      }
    } catch (error) {
  ...
  private async buildAgentsTable(context, printer: ReportPrinter, agentIDs: string[], apiId: string, groupID: string = '') {
    ...
    try {
      ...
      if (groupID) {
        ...
        do{
          const { data: { data: { affected_items, total_affected_items } } } = await context.psafe.api.client.asCurrentUser.request(
            'GET',
        ...
        for (const agentID of agentIDs) {
          try {
            const { data: { data: { affected_items: [agent] } } } = await context.psafe.api.client.asCurrentUser.request(
              'GET',
              `/agents`,
              { 
                params: {
  ...
  private async extendedInformation(
    ...
    try {
      ...
      const agents = await context.psafe.api.client.asCurrentUser.request(
        'GET',
        '/agents',
        { params: { limit: 1 } },
        { apiHostID: apiId }
      ...
      if (section === 'agents' && tab === 'fim') {
        ...
        const lastScanResponse = await context.psafe.api.client.asCurrentUser.request(
          'GET',
        ...
        const syscollectorLists = await Promise.all(
          requestsSyscollectorLists.map(async (requestSyscollector) => {
            try {
              ...
              const responseSyscollector = await context.psafe.api.client.asCurrentUser.request(
                'GET',
  ...
  createReportsModules = this.checkReportsUserDirectoryIsValidRouteDecorator(async (
    ...
    try {
      ...
      createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_DIRECTORY_PATH);
      createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
      createDirectoryIfNotExists(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, context.psafeEndpointParams.hashUsername));
      ...
      await printer.print(context.psafeEndpointParams.pathFilename);

      return response.ok({
        body: {
          success: true,
          message: `Report ${context.psafeEndpointParams.filename} was created`,
        },
      });
    } catch (error) {
      return ErrorResponse(error.message || error, 5029, 500, response);
    }
  },({body:{ agents }, params: { moduleID }}) => `psafe-module-${agents ? `agents-${agents}` : 'overview'}-${moduleID}-${this.generateReportTimestamp()}.pdf`)
  ...
  createReportsGroups = this.checkReportsUserDirectoryIsValidRouteDecorator(async(
    ...
    try {
      ...
      if (components['0']) {
        ...
        const { data: { data: configuration } } = await context.psafe.api.client.asCurrentUser.request(
          'GET',
  ...
  }, ({params: { groupID }}) => `psafe-group-configuration-${groupID}-${this.generateReportTimestamp()}.pdf`)
  ...
  createReportsAgentsConfiguration = this.checkReportsUserDirectoryIsValidRouteDecorator( async (
    ...
    try {
      ...
      let tables = [];
      try {
        wmodulesResponse = await context.psafe.api.client.asCurrentUser.request(
          'GET',
        ...
        for (let section of config.sections) {
          ...
          if (
            ...
            for (let conf of configs) {
              ...
              try {
                if (!conf['name']) {
                  agentConfigResponse = await context.psafe.api.client.asCurrentUser.request(
                    'GET',
  ...
  }, ({ params: { agentID }}) => `psafe-agent-configuration-${agentID}-${this.generateReportTimestamp()}.pdf`)
  ...
  createReportsAgentsInventory = this.checkReportsUserDirectoryIsValidRouteDecorator( async (
    ...
    try {
      ...
      const { hashUsername } = await context.psafe.security.getCurrentUser(request, context);
      createDataDirectoryIfNotExists();
      createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_DIRECTORY_PATH);
      createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
      createDirectoryIfNotExists(path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, hashUsername));
      ...
      const requestInventory = async (agentRequestInventory) => {
        try {
          ...
          const inventoryResponse = await context.psafe.api.client.asCurrentUser.request(
            'GET',
  ...
  }, ({params: { agentID }}) => `psafe-agent-inventory-${agentID}-${this.generateReportTimestamp()}.pdf`)
  ...
  async getReports(
    ...
    try {
      ...
      createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_DIRECTORY_PATH);
      createDirectoryIfNotExists(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH);
      const userReportsDirectoryPath = path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, hashUsername);
  ...
  getReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(async (
    ...
    try {
      log('reporting:getReportByName', `Getting ${context.psafeEndpointParams.pathFilename} report`, 'debug');
      const reportFileBuffer = fs.readFileSync(context.psafeEndpointParams.pathFilename);
      return response.ok({
  ...
  deleteReportByName = this.checkReportsUserDirectoryIsValidRouteDecorator(async (
    ...
    try {
      log('reporting:deleteReportByName', `Deleting ${context.psafeEndpointParams.pathFilename} report`, 'debug');
      fs.unlinkSync(context.psafeEndpointParams.pathFilename);
      log('reporting:deleteReportByName', `${context.psafeEndpointParams.pathFilename} report was deleted`, 'info');
      return response.ok({
        body: { error: 0 },
      });
    } catch (error) {
  ...
  checkReportsUserDirectoryIsValidRouteDecorator(routeHandler, reportFileNameAccessor){
    return (async (
      ...
      try{
        const { username, hashUsername } = await context.psafe.security.getCurrentUser(request, context);
        const userReportsDirectoryPath = path.join(PSAFE_DATA_DOWNLOADS_REPORTS_DIRECTORY_PATH, hashUsername);
        ...
        return await routeHandler.bind(this)({...context, psafeEndpointParams: { hashUsername, filename, pathFilename }}, request, response);
      }catch(error){
