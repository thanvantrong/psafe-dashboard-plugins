/*
 * Psafe app - Class for UI Logs functions
...
import { PSAFE_UI_LOGS_RAW_PATH } from '../../../common/constants';
  ...
  async getUiLogs(response: KibanaResponseFactory) {
    try {
      return uiLogger.initDirectory().then(async () => {
        if (!uiLogger.checkFileExist(PSAFE_UI_LOGS_RAW_PATH)) {
          return response.ok({
            body: {
              error: 0,
              rawLogs: [],
          ...
          let arrayLog = await this.getUiFileLogs(PSAFE_UI_LOGS_RAW_PATH);
          return response.ok({
            body: {
              error: 0,
