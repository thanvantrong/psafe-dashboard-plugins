...
const elasticServer = process.env.PSAFE_ELASTIC_IP || 'localhost';
chai.should();
...
const index = `psafe-alerts-2018.${month >= 10 ? month : `0${month}`}.${
  day >= 10 ? day : `0${day}`
}`;
...
const syscheck = async agentID => {
  ...
  sample.source.should.be.eql('/var/optit/logs/alerts/alerts.json');
  sample.location.should.be.eql('syscheck');
  sample.cluster.should.be.a('object');
  sample.cluster.name.should.be.eql('psafe');
