...
const { PSAFE_ALERTS_PATTERN, PLUGIN_PLATFORM_REQUEST_HEADERS } = require('../../common/constants');
...
describe('psafe-elastic', () => {
  describe('Checking index patterns', () => {
    it('GET /elastic/known-fields/{pattern}', async () => {
      const res = await needle(
        'get',
        `${kibanaServer}:5601/elastic/known-fields/${PSAFE_ALERTS_PATTERN}`,
        {},
        headers
      );
      ...
      res.body.output._id.should.be.eql(`index-pattern:${PSAFE_ALERTS_PATTERN}`);
    });
  });
  ...
  describe('Checking visualization composers', () => {
    it('GET /elastic/visualizations/{tab}/{pattern}', async () => {
      const res = await needle(
        'get',
        `${kibanaServer}:5601/elastic/visualizations/overview-general/${PSAFE_ALERTS_PATTERN}`,
        {},
        headers
      );
      res.body.acknowledge.should.be.eql(true);
    ...
    it('POST /elastic/visualizations/{tab}/{pattern}', async () => {
      const res = await needle(
        'post',
        `${kibanaServer}:5601/elastic/visualizations/cluster-monitoring/${PSAFE_ALERTS_PATTERN}`,
  ...
  describe('Checking template and index pattern existance', () => {
    it('GET /elastic/template/{pattern}', async () => {
      const res = await needle(
        'get',
        `${kibanaServer}:5601/elastic/template/${PSAFE_ALERTS_PATTERN}`,
        {},
        headers
      );
      res.body.statusCode.should.be.eql(200);
      res.body.status.should.be.eql(true);
      res.body.data.should.be.eql(`Template found for ${PSAFE_ALERTS_PATTERN}`);
    });

    it('GET /elastic/index-patterns/{pattern}', async () => {
      const res = await needle(
        'get',
        `${kibanaServer}:5601/elastic/index-patterns/${PSAFE_ALERTS_PATTERN}`,
        {},
        headers
      );
      res.body.statusCode.should.be.eql(200);
