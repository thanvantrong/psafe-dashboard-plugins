...
export function ApiProvider({ getService, getPageObjects }) {
  ...
  class ApiPage {
    ...
    async completeApiForm(type = 'apiConfig') {
      ...
      await testSubjects.setValue(`${type}Username`, 'psafe');
      log.debug('insert the user');
      await testSubjects.setValue(`${type}Password`, 'psafe');
    ...
    async checkTabDisabled(tab) {
      ...
      expect(await browser.getCurrentUrl()).to.contain('app/psafe#/settings');
    }
    ...
    async checkIfAllTabsAreDisables() {
      await this.checkTabDisabled('paMenuOverview');
      await this.checkTabDisabled('paMenuManagement');
      await this.checkTabDisabled('paMenuAgents');
      await this.checkTabDisabled('paMenuDiscover');
      await this.checkTabDisabled('paMenuDevTools');
    }
    ...
    async navigateToApiSetting() {
      await testSubjects.click('paMenuSettings');
